...
newnames<- c("Event.Nr", paste0("BIO_", paste0(leda_bioames[2:21])))
...
tmp.df4_sum_full_bio<- left_join(tmp.df4_sum_full,  db_leda_bio)
...
db_HR_bio$trigger<- if_else(db_HR_bio$trigger_200 == TRUE, as.numeric(db_HR_bio$trigger)-128, as.numeric(db_HR_bio$trigger))


unique((as.numeric(db_HR_bio$trigger)))
...
db_HR_bio1<- subset(db_HR_bio,db_HR_bio$trigger< 100)
...
db_HR_bio1no328<- db_HR_bio1no328 %>%
  group_by(as.numeric(ssid))%>%
  mutate(tNo = -6:49)

db_HR_bio328<-subset(db_HR_bio1, db_HR_bio1$ssid== 328)

db_HR_bio328<- db_HR_bio328 %>%
  group_by(as.numeric(ssid))%>%
  mutate(tNo = -6:24)
...
db_HR_bio2<- select(db_HR_bio2, c(timestart,Mean_HR, tNo, ssid))

db_HR_bio2$`as.numeric(ssid)`<- NULL
...
db_HR_bio1$tNo<- if_else(db_HR_bio1$tNo< 0, as.numeric(db_HR_bio1$tNo), as.numeric(db_HR_bio1$tNo+1))
