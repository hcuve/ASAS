...
db_HR_bio1<- subset(db_HR_bio,db_HR_bio$trigger2< 100)
...
db_HR_bio1no328<- subset(db_HR_bio1no328, trigger2> 15 & trigger2<72)
...
```{r}
db_HR_bio2<- db_HR_bio328 %>%
  group_by(ssid)%>%
  mutate(tNo = -6:(n()-7))
unique(db_HR_bio328$tNo)
```
...
write_csv(db_HR_bio_final, 'db_HR_bio_final_346_355.csv')
...
tmp.df4_sum_full_bio_hr <- left_join(tmp.df4_sum_full_bio, db_HR_bio_final, by = c('ssid', 'tNo'))
