...
eigen_first%>%
  ...
  mutate(eig_obs_dim = if_else(component == "comp 1", 
                               res.mfa_bio_vs_self2_k2$eig[1],
                               res.mfa_bio_vs_self2_k2$eig[2])) %>% #create 95% to descrive interval around the observed estimate
  ...
  geom_errorbar(aes(xmin = perc_5, xmax=perc_95, y = 1), color = "gray30")+
  geom_point(aes(x = eig_obs_dim, y = 1),size = 2, color = "red", alpha = .3)+
  ...
  facet_grid(~component)+
  ggtitle("distribution of boostraped eigen values for components 
...
use this in the resampling bit
```{r}
...
eigen_first %>%
  ...
  mutate(perc_5 = quantile(`percentage of variance`,.025, na.rm = TRUE), 
         perc_95 = quantile(`percentage of variance`,.975, na.rm = TRUE))%>%
  ungroup()%>%
  mutate(perc_obs_dim = if_else(component == "comp 1", 
                               res.mfa_bio_vs_self2_k2$eig[1,2],
                               res.mfa_bio_vs_self2_k2$eig[2,2])) %>% #
  ggplot(aes(`percentage of variance`))+
  geom_histogram(aes(y=..density..), alpha=0.5,
                 position="identity")+
  geom_density(alpha=.2) +
  geom_errorbarh(aes(xmax = perc_95, xmin = perc_5, y = .3), color = "gray30")+
  geom_point(aes(x = perc_obs_dim,y = .3),size = 3, color = "red")+
  facet_grid(~component)+
...
res.mfa_bio_vs_self2_scale$eig[2,3]
```
...
cor.test_obsmfa<- as.data.frame(res.mfa_bio_vs_self2_k2$quanti.var$cor) %>%
   mutate(variable = as.factor(substr(rownames(as.data.frame(res.mfa_bio_vs_self2_k2$quanti.var$cor)), 1,6)))%>%
 gather(dimension, value_dimenion, -variable)


cor_first1<- cor_first %>%
  mutate(variable = as.factor(substr(rownames(cor_first), 1,6)))%>%
    gather(dimension, value_dimenion, -variable, -sim_no) %>%
  group_by(variable,dimension)%>%
...
left_join(cor_first1, cor.test_obsmfa, by = c("variable", "dimension")) %>%
  ...
  facet_grid(variable~dimension)+
  # geom_vline(xintercept = .5, color = "red", linetype = "dashed", alpha = .1, size = 1)+

  theme_classic()

mfa_boot_corfirst<- left_join(cor_first1, cor.test_obsmfa, by = c("variable", "dimension")) 

mfa_boot_corfirst$measure<- if_else(mfa_boot_corfirst$variable == "log_BI", "SCR",
        if_else(mfa_boot_corfirst$variable == "Bio_Me", "HR",
                if_else(mfa_boot_corfirst$variable == "pup_ba", "pupil",
                if_else(mfa_boot_corfirst$variable == "arousa", "arousal",
                if_else(mfa_boot_corfirst$variable == "valenc", "valence",NULL)
                ))))

mfa_boot_corfirst$variable
left_join(cor_first1, cor.test_obsmfa, by = c("variable", "dimension"))


mfa_boot_corfirst%>%
  subset(dimension == "Dim.1")%>%
  ggplot(aes(value_dimenion.x, color = measure)) +
  geom_histogram(aes(y=..density..), alpha=0.5,
                 position="identity")+
  # geom_density(alpha=.2) +
...
res.mfa_bio_vs_self2_k2$partial.axes$cor

partials_first_1_obs<- as.data.frame(res.mfa_bio_vs_self2_k2$partial.axes$cor) %>%
   mutate(dimension_partial = as.factor(substr(rownames(as.data.frame(res.mfa_bio_vs_self2_k2$partial.axes$cor)), 1,4)),
           group = as.factor(substr(rownames(as.data.frame(res.mfa_bio_vs_self2_k2$partial.axes$cor)), 5,9)))%>%
 gather(dimension, value_dimenion,-group,-dimension_partial)




partials_first_1boot<- partials_first%>%
  mutate(dimension_partial = as.factor(substr(rownames(partials_first), 1,4)),
         group = as.factor(substr(rownames(partials_first), 5,9)))%>%
   gather(dimension, value_dimenion, -group, -sim_no, -dimension_partial) %>%
   group_by(dimension_partial,dimension,group)%>%
   mutate(perc_5 = quantile(value_dimenion,.025, na.rm = TRUE),
         perc_95 = quantile(value_dimenion,.975, na.rm = TRUE))

left_join(partials_first_1boot,partials_first_1_obs,by = c("dimension_partial", "group", "dimension")) %>%
  group_by(dimension_partial,dimension, group)%>%
  subset(dimension_partial == "Dim1")%>%
  ...
  geom_histogram(aes(y=..density..), alpha=0.5,
                 position="identity")+
  geom_density(alpha=.2) +
  geom_errorbarh(aes(xmax = perc_95, xmin = perc_5, y =-5 ), color = "gray30")+
  geom_point(aes(x = value_dimenion.y, y = -5))+
    geom_vline(xintercept = 0, color = "red", linetype = "dashed", alpha = .3, size = 1)+
  
  facet_grid(dimension~group)+
  
  geom_vline(xintercept = 0, color = "red", linetype = "dashed", alpha = .5)+
  theme_classic()+
  ggtitle("partial to dimension 1")


left_join(partials_first_1boot,partials_first_1_obs,by = c("dimension_partial", "group", "dimension")) %>%
  group_by(dimension_partial,dimension, group)%>%
  subset(dimension_partial == "Dim2")%>%
   # mutate(perc_5 = quantile(Dim.1,.025, na.rm = TRUE),
   #       perc_95 = quantile(Dim.1,.975, na.rm = TRUE))%>%
  ggplot(aes(value_dimenion.x)) +
  geom_histogram(aes(y=..density..), alpha=0.5,
  ...
  geom_vline(xintercept = 0, color = "red", linetype = "dashed", alpha = .5)+
  theme_classic()+
  ggtitle("partial to dimension 2")
...
groups_first_obs<- as.data.frame(res.mfa_bio_vs_self2_k2$group$correlation) %>%
   mutate(group = as.factor(substr(rownames(as.data.frame(res.mfa_bio_vs_self2_k2$group$correlation)), 1,4)))%>%
 gather(dimension, value_dimenion_obs,-group)
...
groups_first_1boot<- groups_first%>%
...
?boot::boot.ci
boot::boot.ci(groups_first_1boot$value_dimenion, type = "bca")[1]
?bootBCa
bootBCa(.5, groups_first_1boot$value_dimenion, n = 2600)[1]
left_join(groups_first_1boot, groups_first_obs,by = c("group", "dimension"))%>%
  ...
  # bootsrap confidence intervals don't include point estimate
# seems like a consequnce of resampling a biased statistocs
...
# try plooting with corrected bca
# ?bca
# groups_first_1boot_CIcorrected<- groups_first%>%
#   mutate(group = as.factor(substr(rownames(groups_first), 1,4)))%>%
#   gather(dimension, value_dimenion,-group,-sim_no)%>%
#   # -perc_5,-perc_95, 
#    group_by(group,dimension)%>%
#   subset(!is.na(value_dimenion))%>%
#    mutate(perc_5 = bca(value_dimenion)[1],
#          perc_95 = bca(value_dimenion)[2])


groups_first_1boot_CIcorrected2<- groups_first%>%
  mutate(group = as.factor(substr(rownames(groups_first), 1,4)))%>%
  gather(dimension, value_dimenion,-group,-sim_no)
...
bcatest<- partials_first_1boot%>%
  subset(!is.na(value_dimenion))

bcatest<- !is.na(bcatest)

bca(bcatest$value_dimenion)[1]
...
to see the reshufllfing for the hierarqucal data see separate script - paper_analyses_subj1
...
eigen_first_bysubj%>%
  ...
  ungroup()%>%
  mutate(eig_obs_dim = if_else(component == "comp 1", 
                              res.mfa_agg_bysubj$eig[1],
                              res.mfa_agg_bysubj$eig[2])) %>% #create 95% to descrive interval around the observed estimate
  # also store observed eige for each dimensions
  # mutate(probabcheck= sum(eigenvalue>=res.mfa_bio_vs_self2_k2$eig[1])/n())
  ggplot(aes(eigenvalue))+
  geom_histogram(aes(y=..density..), alpha = .3) +  # scale histogram y
...
```{r}
res.mfa_agg_bysubj
cor.test_obsmfa_bysubj<- as.data.frame(res.mfa_agg_bysubj$quanti.var$cor) %>%
   mutate(variable = as.factor(substr(rownames(as.data.frame(res.mfa_agg_bysubj$quanti.var$cor)), 1,6)))%>%
 gather(dimension, value_dimenion, -variable)


cor_first_bysubj1<- cor_first_bysubj %>%
  mutate(variable = as.factor(substr(rownames(cor_first), 1,6)))%>%
    gather(dimension, value_dimenion, -variable, -sim_no) %>%
  group_by(variable,dimension)%>%
   mutate(perc_5 = quantile(value_dimenion,.025, na.rm = TRUE),
...
left_join(cor_first_bysubj1, cor.test_obsmfa_bysubj, by = c("variable", "dimension")) %>%
  ggplot(aes(value_dimenion.x)) +
  geom_histogram(aes(y=..density..), alpha=0.5,
                 position="identity")+
  geom_density(alpha=.2) +
...
groups_first_bysubj_obs<- as.data.frame(res.mfa_agg_bysubj$group$correlation) %>%
   mutate(group = as.factor(substr(rownames(as.data.frame(res.mfa_agg_bysubj$group$correlation)), 1,4)))%>%
 gather(dimension, value_dimenion_obs,-group)

groups_first_bysubj_boot<- groups_first_bysubj%>%
  mutate(group = as.factor(substr(rownames(groups_first), 1,4)))%>%
  gather(dimension, value_dimenion,-group,-sim_no)
  # -perc_5,-perc_95, 
 

left_join(groups_first_bysubj_boot, groups_first_bysubj_obs,by = c("group", "dimension"))%>%
    group_by(group,dimension)%>%
   mutate(perc_5 = bootBCa(mean(value_dimenion_obs), value_dimenion, n = 51)[1],
         perc_95 = bootBCa(mean(value_dimenion_obs), value_dimenion, n = 51)[2])%>%
  ...
  geom_density(alpha=.2) +
   ...
   geom_errorbarh(aes(xmax = perc_95, xmin = perc_5, y =-1 ), color = "gray30")+
...
eigen_first_bysubj_reshuf%>%
  mutate(component =as.factor(substr(rownames(eigen_first_bysubj_reshuf), 1,6)))%>% #retrieve names for components
  ...
  mutate(eig_obs_dim = if_else(component == "comp 1", 
                               res.mfa_agg_bysubj$eig[1],
                               res.mfa_agg_bysubj$eig[2])) %>% #store observed to visualise ion the null
  mutate(prob = paste(((sum(eigenvalue>=eig_obs_dim))/n())*100, paste0("%"))) %>%
  ggplot(aes(eigenvalue))+
  geom_histogram(aes(y=..density..),bins = 500, alpha = .3) +  # scale histogram y
  geom_density(col = "red", alpha = .001)+
  # facet_grid(~component)+
    geom_vline(aes(xintercept = eig_obs_dim),size = 2, color = "red", linetype = "dashed", alpha = .3)+
  geom_point(aes(x = eig_obs_dim, y = 1, shape = prob),size = 3,  color = "red", alpha = .5)+

  theme_classic()+

  # annotate(aes(text, label = prob))
...
colnames(cor_first_reshuf)
cor_first_bysubj_reshuf1<- cor_first_bysubj_reshuf %>%
...
# this mighgt have to do with the wrong test
left_join(cor_first_bysubj_reshuf1, cor.test_obsmfa_bysubj, by = c("variable", "dimension")) %>%
  group_by(variable,dimension)%>%
     mutate(prob1 = paste(((sum(abs(value_dimenion.x)>=abs(value_dimenion.y)))/n())*100, paste0("%")))%>%
   mutate(prob = if_else(((sum(abs(value_dimenion.x)>=abs(value_dimenion.y))/n())*100)<5, "p < .05", "ns")) %>%
  # the line above computes the probability that the correlation is more extreme (in magnitutde) thn the observed one
  ...
  # geom_point(aes(x = value_dimenion.y, y = -2))+
    # geom_vline(xintercept = 0, color = "red", linetype = "dashed", alpha = .3, size = 1)+
  facet_grid(variable~dimension)+
   geom_vline(aes(xintercept = value_dimenion.y),size = 2, color = "red", linetype = "dashed", alpha = .3)+
  geom_point(aes(x = value_dimenion.y, y = 1, shape = prob),size = 3,  color = "red", alpha = .5)+
  # geom_vline(xintercept = .5, color = "red", linetype = "dashed", alpha = .1, size = 1)+
...
```{r}
partials_first_1_bysubj_obs<- as.data.frame(res.mfa_agg_bysubj$partial.axes$cor) %>%
   mutate(dimension_partial = as.factor(substr(rownames(as.data.frame(res.mfa_agg_bysubj$partial.axes$cor)), 1,4)),
           group = as.factor(substr(rownames(as.data.frame(res.mfa_agg_bysubj$partial.axes$cor)), 5,9)))%>%
 gather(dimension, value_dimenion,-group,-dimension_partial)


partials_first_bysubj_reshuf

partials_first_bysubj_reshuf_1<- partials_first_bysubj_reshuf %>%
  mutate(dimension_partial = as.factor(substr(rownames(partials_first_bysubj_reshuf), 1,4)),
         group = as.factor(substr(rownames(partials_first_bysubj_reshuf), 5,9)))%>%
   gather(dimension, value_dimenion, -group, -sim_no, -dimension_partial) %>%
   group_by(dimension_partial,dimension,group)%>%
   mutate(perc_5 = quantile(value_dimenion,.025, na.rm = TRUE),
         perc_95 = quantile(value_dimenion,.975, na.rm = TRUE))%>%
...
left_join(partials_first_bysubj_reshuf_1,partials_first_1_bysubj_obs,by = c("dimension_partial", "group", "dimension")) %>%
  group_by(dimension_partial,dimension, group)%>%
   subset(!is.na(value_dimenion.x))%>%
   # group_by(variable,dimension)%>%
     mutate(prob1 = paste(((sum(abs(value_dimenion.x)>=abs(value_dimenion.y)))/n())*100, paste0("%")))%>%
   mutate(prob = if_else(((sum(abs(value_dimenion.x)>=abs(value_dimenion.y))/n())*100)<5, "p < .05", "ns")) %>%
  subset(dimension_partial == "Dim1")%>%
...
groups_first_bysubj_reshuf
groups_first_bysubj_reshuf<- groups_first_bysubj_reshuf%>%
  mutate(group = as.factor(substr(rownames(groups_first_reshuf), 1,4)))


group_cordf_bysubj<- as.data.frame(res.mfa_agg_bysubj$group$correlation)
group_cordf_bysubj$group<-as.factor(substr(rownames(group_cordf_bysubj), 1,4)) 

group_cordf_bysubj1<- group_cordf_bysubj%>%
gather(dimension, value_dimenion, - group)


 
# View(groups_first_reshuf)
groups_first_bysubj_reshuf1<- groups_first_bysubj_reshuf %>%
  gather(dimension, value_dimenion, -group, -sim_no) %>%
  subset(!is.na(value_dimenion))
  
 left_join(groups_first_bysubj_reshuf1, group_cordf_bysubj1, c("group", "dimension"))%>%
   group_by(group,dimension)%>%
   mutate(perc_5 = quantile(value_dimenion.x,.025, na.rm = TRUE),
         perc_95 = quantile(value_dimenion.x,.975, na.rm = TRUE))%>%
     mutate(prob1 = paste(((sum(abs(value_dimenion.x)>=abs(value_dimenion.y)))/n())*100, paste0("%")))%>%
   mutate(prob = if_else(((sum(abs(value_dimenion.x)>=abs(value_dimenion.y))/n())*100)<5, "p < .05", "ns")) %>%
  ...
  geom_density(alpha=.2) +
  facet_grid(dimension~group)+
     geom_vline(aes(xintercept = value_dimenion.y), linetype = "dashed", color = "red")+
...
resamp_1st_bysubj_cat_val<- data.frame(matrix(NA, nrow = 1, ncol = 12))
 ...
 cnames_resamp_subj1<- names(resampled_temp_bysubj_cat_val)
colnames(resamp_1st_bysubj_cat_val)<- cnames_resamp_subj1
...
eigen_first_bysubj_cat_val%>%
  ...
  ungroup()%>%
  mutate(eig_obs_dim = if_else(component == "comp 1", 
                              res.mfa_agg_bysubj_cat_val$eig[1],
                              res.mfa_agg_bysubj_cat_val$eig[2])) %>% #create 95% to descrive interval around the observed estimate
  # also store observed eige for each dimensions
  # mutate(probabcheck= sum(eigenvalue>=res.mfa_bio_vs_self2_k2$eig[1])/n())
  ggplot(aes(eigenvalue))+
  geom_histogram(aes(y=..density..), alpha = .3) +  # scale histogram y
...
variables and dimension correlations

```{r}
cor.test_obsmfa_bysubj_cat_val<- as.data.frame(res.mfa_agg_bysubj_cat_val$quanti.var$cor) %>%
   mutate(variable = as.factor(substr(rownames(as.data.frame(res.mfa_agg_bysubj_cat_val$quanti.var$cor)), 1,6)))%>%
 gather(dimension, value_dimenion, -variable)


cor_first_bysubj_cat_val1<- cor_first_bysubj_cat_val %>%
...
left_join(cor_first_bysubj_cat_val1, cor.test_obsmfa_bysubj_cat_val, by = c("variable", "dimension")) %>%
  ggplot(aes(value_dimenion.x)) +
  geom_histogram(aes(y=..density..), alpha=0.5,
                 position="identity")+
  geom_density(alpha=.2) +
...
save.image("subjobjPCAfeb93.RData")
...
eigen_first_bysubj_reshuf_cat_val%>%
  ...
  ungroup()%>%
    group_by(component)%>%
  mutate(eig_obs_dim = if_else(component == "comp 1", 
                               res.mfa_agg_bysubj_cat_val$eig[1],#always update these two lines
                               res.mfa_agg_bysubj_cat_val$eig[2])) %>% #store observed to visualise ion the null
  mutate(prob = paste(((sum(eigenvalue>=eig_obs_dim))/n())*100, paste0("%"))) %>%
  ggplot(aes(eigenvalue))+
  geom_histogram(aes(y=..density..),bins = 500, alpha = .3) +  # scale histogram y
  geom_density(col = "red", alpha = .001)+
...
colnames(cor_first_reshuf)
cor_first_bysubj_reshuf_cat_val1<- cor_first_bysubj_reshuf_cat_val %>%
...
# this mighgt have to do with the wrong test
left_join(cor_first_bysubj_reshuf_cat_val1, cor.test_obsmfa_bysubj_cat_val, by = c("variable", "dimension")) %>%
  group_by(variable,dimension)%>%
     mutate(prob1 = paste(((sum(abs(value_dimenion.x)>=abs(value_dimenion.y)))/n())*100, paste0("%")))%>%
   mutate(prob = if_else(((sum(abs(value_dimenion.x)>=abs(value_dimenion.y))/n())*100)<5, "p < .05", "ns")) %>%
  # the line above computes the probability that the correlation is more extreme (in magnitutde) thn the observed one
...
groups_first_bysubj_reshuf_cat_val
groups_first_bysubj_reshuf_cat_val<- groups_first_bysubj_reshuf_cat_val%>%
  mutate(group = as.factor(substr(rownames(groups_first_bysubj_reshuf_cat_val), 1,4)))


group_cordf_bysubj_cat_val_obs<- as.data.frame(res.mfa_agg_bysubj_cat_val$group$correlation)
group_cordf_bysubj_cat_val_obs$group<-as.factor(substr(rownames(group_cordf_bysubj_cat_val_obs), 1,4)) 

group_cordf_bysubj_cat_val_obs1<- group_cordf_bysubj_cat_val_obs%>%
gather(dimension, value_dimenion, - group)


 
# View(groups_first_reshuf)
groups_first_bysubj_reshuf_cat_val1<- groups_first_bysubj_reshuf_cat_val %>%
  gather(dimension, value_dimenion, -group, -sim_no) %>%
  subset(!is.na(value_dimenion))
  
 left_join(groups_first_bysubj_reshuf_cat_val1, group_cordf_bysubj_cat_val_obs1, c("group", "dimension"))%>%
   group_by(group,dimension)%>%
   mutate(perc_5 = quantile(value_dimenion.x,.025, na.rm = TRUE),
         perc_95 = quantile(value_dimenion.x,.975, na.rm = TRUE))%>%
     mutate(prob1 = paste(((sum(abs(value_dimenion.x)>=abs(value_dimenion.y)))/n())*100, paste0("%")))%>%
...
eigen_first_bystim_cat_val%>%
  ...
  ungroup()%>%
  mutate(eig_obs_dim = if_else(component == "comp 1", 
                              res.mfa_agg_stim $eig[1],
                              res.mfa_agg_stim$eig[2])) %>% #create 95% to descrive interval around the observed estimate
  # also store observed eige for each dimensions
  # mutate(probabcheck= sum(eigenvalue>=res.mfa_bio_vs_self2_k2$eig[1])/n())
  ggplot(aes(eigenvalue))+
  geom_histogram(aes(y=..density..), alpha = .3) +  # scale histogram y
...
```{r}
cor.test_obsmfa_bystim_cat_val<- as.data.frame(res.mfa_agg_stim$quanti.var$cor) %>%
   mutate(variable = as.factor(substr(rownames(as.data.frame(res.mfa_agg_stim$quanti.var$cor)), 1,6)))%>%
 gather(dimension, value_dimenion, -variable)


cor_first_bystim_cat_val1<- cor_first_bystim_cat_val %>%
...
left_join(cor_first_bystim_cat_val1, cor.test_obsmfa_bystim_cat_val, by = c("variable", "dimension")) %>%
  ggplot(aes(value_dimenion.x)) +
  geom_histogram(aes(y=..density..), alpha=0.5,
                 position="identity")+
  geom_density(alpha=.2) +
...
groups_first_bystim_obs<- as.data.frame(res.mfa_agg_stim$group$correlation) %>%
   mutate(group = as.factor(substr(rownames(as.data.frame(res.mfa_agg_stim$group$correlation)), 1,4)))%>%
 gather(dimension, value_dimenion_obs,-group)

groups_first_bystim_cat_val_boot<- groups_first_bystim_cat_val%>%
  mutate(group = as.factor(substr(rownames(groups_first_bystim_cat_val), 1,4)))%>%
  gather(dimension, value_dimenion,-group,-sim_no)
  # -perc_5,-perc_95, 
 

left_join(groups_first_bystim_cat_val_boot, groups_first_bystim_obs,by = c("group", "dimension"))%>%
    group_by(group,dimension)%>%
   mutate(perc_5 = bootBCa(mean(value_dimenion_obs), value_dimenion, n = 51)[1],
         perc_95 = bootBCa(mean(value_dimenion_obs), value_dimenion, n = 51)[2])%>%
...
eigen_first_bystimj_reshuf%>%
  ...
  mutate(eig_obs_dim = if_else(component == "comp 1", 
                               res.mfa_agg_stim$eig[1],
                               res.mfa_agg_stim$eig[2])) %>% #store observed to visualise ion the null
  mutate(prob = paste(((sum(eigenvalue>=eig_obs_dim))/n())*100, paste0("%"))) %>%
  ggplot(aes(eigenvalue))+
  geom_histogram(aes(y=..density..),bins = 500, alpha = .3) +  # scale histogram y
  geom_density(col = "red", alpha = .001)+
...
```{r}
res.mfa_agg_bysubj$quanti.var$cor
cor_first_bystimj_reshuf


colnames(cor_first_reshuf)
cor_first_bystimj_reshuf_cat_val1<- cor_first_bystimj_reshuf %>%
  mutate(variable = as.factor(substr(rownames(cor_first_bystimj_reshuf), 1,6)))%>%
    gather(dimension, value_dimenion, -variable, -sim_no) %>%
  group_by(variable,dimension)%>%
   mutate(perc_5 = quantile(value_dimenion,.025, na.rm = TRUE),
         perc_95 = quantile(value_dimenion,.975, na.rm = TRUE))%>%
...
# this mighgt have to do with the wrong test
left_join(cor_first_bystimj_reshuf_cat_val1, cor.test_obsmfa_bystim_cat_val, by = c("variable", "dimension")) %>%
  group_by(variable,dimension)%>%
     mutate(prob1 = paste(((sum(abs(value_dimenion.x)>=abs(value_dimenion.y)))/n())*100, paste0("%")))%>%
   mutate(prob = if_else(((sum(abs(value_dimenion.x)>=abs(value_dimenion.y))/n())*100)<5, "p < .05", "ns")) %>%
  # the line above computes the probability that the correlation is more extreme (in magnitutde) thn the observed one
...
groups_first_bystimj_reshuf
groups_first_bystimj_reshuf<- groups_first_bystimj_reshuf%>%
  mutate(group = as.factor(substr(rownames(groups_first_bystimj_reshuf), 1,4)))


group_cordf_bystim_cat_val_obs<- as.data.frame(res.mfa_agg_stim$group$correlation)
group_cordf_bystim_cat_val_obs$group<-as.factor(substr(rownames(group_cordf_bystim_cat_val_obs), 1,4)) 

group_cordf_bystim_cat_val_obs1<- group_cordf_bystim_cat_val_obs%>%
gather(dimension, value_dimenion, - group)


 
# View(groups_first_reshuf)
groups_first_bystimj_reshuf1<- groups_first_bystimj_reshuf %>%
  gather(dimension, value_dimenion, -group, -sim_no) %>%
  subset(!is.na(value_dimenion))
  
 left_join(groups_first_bystimj_reshuf1, group_cordf_bystim_cat_val_obs1, c("group", "dimension"))%>%
   group_by(group,dimension)%>%
   mutate(perc_5 = quantile(value_dimenion.x,.025, na.rm = TRUE),
         perc_95 = quantile(value_dimenion.x,.975, na.rm = TRUE))%>%
     mutate(prob1 = paste(((sum(abs(value_dimenion.x)>=abs(value_dimenion.y)))/n())*100, paste0("%")))%>%
