...
partials_first <- res.mfa_bio_vs_self2_k2[["partial.axes"]][["contrib"]]
partials_first[,]<- NA
partials_first
...
for (i in 1:1500) {
  ...
  resampled_temp<-  resample_data(clustering_new_pre_PCA3.1 [,c(1,2,3, 5:9)], N = 51, ID_labels = "ssid")
...
partials_first<- as.data.frame(res.mfa_bio_vs_self2_k2$partial.axes$cor)
partials_first$sim_no = i
...
eigen_first%>%
  mutate(component =as.factor(substr(rownames(eigen_first), 1,6)))%>%
    subset(!is.na(eigenvalue))%>%
  ggplot(aes(eigenvalue))+
  geom_histogram(alpha = .2)+
  facet_grid(~component)+
  geom_vline(xintercept = res.mfa_bio_vs_self2_k2$eig[1], linetype= "dashed")+
    geom_vline(xintercept = res.mfa_bio_vs_self2_k2$eig[2], linetype = "dotted")+
  theme_classic()
...
eigen_first%>%
  ...
  mutate(perc_5 = quantile(eigenvalue,.05, na.rm = TRUE),
         perc_95 = quantile(eigenvalue,.95,, na.rm = TRUE))%>%
  ggplot(aes(eigenvalue))+
  geom_histogram(alpha = .2)+
    geom_density()+
  # facet_grid(~component)+
  geom_point(x = res.mfa_bio_vs_self2_k2$eig[1],y = 10,size = 2, color = "red")+
  # geom_errorbar(aes(xmin = min(perc_5), xmax=max(perc_95), ymin = 10, ymax = 12 ))
...
eigen_first%>%
  ...
  mutate(perc_5 = quantile(`percentage of variance`,.05, na.rm = TRUE),
         perc_95 = quantile(`percentage of variance`,.95,, na.rm = TRUE))%>%
  ...
  # facet_grid(~component)+
      geom_errorbarh(aes(xmax = perc_95, xmin = perc_5, y = .3,ymin = .2, ymax = .4), color = "gray30")+
...
# compute a direct test
# we can check the probability of getting a vakue equal to or more extreme than our test

eigen_first%>%
  mutate(component =as.factor(substr(rownames(eigen_first), 1,6)))%>%
    subset(!is.na(`percentage of variance`) & component == "comp 1" )%>%
  mutate(extreme_eigen_log = eigenvalue>= res.mfa_bio_vs_self2_k2$eig[1])%>%
  mutate(extreme_eigen_log_prob = (sum(extreme_eigen_log))/n()) %>%
  summarise_at(c("extreme_eigen_log_prob"), mean, na.rm = TRUE)

# about 50%

# one main difference is that tests checl whether the intervak incldues a ceain value (like zero for eigen vectors or 1 for eigen valkues)
# so not sure if the test abopve is appropriate withouth correcting for reversals
...
eigen_first%>%
  ...
  geom_histogram(alpha = .2)+
  # facet_grid(~component)+
  geom_vline(xintercept = res.mfa_bio_vs_self2_k2$eig[1,2], linetype= "dashed")+
    # geom_vline(xintercept = res.mfa_bio_vs_self2_k2$eig[2], linetype = "dotted")+
  theme_classic()
...
cor_first %>%
 ...
 res.mfa_bio_vs_self2_k2$quanti.var$cor
...
unique(as.character(testres$ssid))

mean(testres$pup_basCor)
mean(clustering_new_pre_PCA3.1$pup_basCor)
...
# okay this fucntion seems to work
```
...
clustering_new_pre_PCA3.1.scale_aft_agg_bysid<- clustering_new_pre_PCA3.1.scale_agg_bysid%>%
 ...
 FactoMineR::MFA(clustering_new_pre_PCA3.1.scale_agg_bysid, 
               group = c(1, 3, 2), 
               type = c("n", "s", "s"),
               # "s", "s", "s"
               name.group = c("stim","auton","subj"),
...
# now keep participant just cast pupil etc
clustering_new_pre_PCA3.1.scale
...
colnames(clustering_new_pre_PCA3.1.scale)
clustering_new_pre_PCA3.1.scale$pup_basCor<- clustering_new_pre_PCA3.1.scale$pup_basCor[,1]

clustering_new_pre_PCA3.1.scale$Bio_Mean_HR_dif<- clustering_new_pre_PCA3.1.scale$Bio_Mean_HR_dif[,1]

clustering_new_pre_PCA3.1.scale$log_BIO_CDA.PhasicMax<- clustering_new_pre_PCA3.1.scale$log_BIO_CDA.PhasicMax[,1]


clustering_new_pre_PCA3.1.scale$arousal<- clustering_new_pre_PCA3.1.scale$arousal[,1]

clustering_new_pre_PCA3.1.scale$valence<- clustering_new_pre_PCA3.1.scale$valence[,1]


clustering_new_pre_PCA3.1.scale_dcast<- data.table::dcast(setDT(clustering_new_pre_PCA3.1.scale), 
                    ssid  ~ stimDescription, 
                    value.var = c('pup_basCor',
                'Bio_Mean_HR_dif', 'log_BIO_CDA.PhasicMax', 'arousal','valence'), fun.aggregate = mean)


colnames(clustering_new_pre_PCA3.1.scale_dcast)

res.mfadcast <- FactoMineR::MFA(clustering_new_pre_PCA3.1.scale_dcast[,
                                                                      2:ncol(clustering_new_pre_PCA3.1.scale_dcast)], 
               group = c(52, 52, 52, 52, 52), 
               type = c("s", "s", "s", "s", "s"),
               # "s", "s", "s"
               name.group = c("pup","hr", "scr", "ar", "vl"),
               # num.group.sup = c(1),
               # ind.sup = c(1,2),
               graph = TRUE)
...
clustering_new_pre_PCA3$getindaxis<- testget[["coord"]][,1]
clustering_new_pre_PCA3$getindaxis2<- testget[["coord"]][,2]

clustering_new_pre_PCA3%>%
  group_by(ssid)%>%
  summarise_if(is.numeric, mean, na.rm = TRUE)%>%
  ggplot(aes(arousal, getindaxis))+
  geom_point()+
  ggpubr::stat_cor()+
  xlim(-,3.3)
...
clustering_new_pre_PCA3%>%
  group_by(ssid, arousal_3cat)%>%
  summarise_if(is.numeric, mean, na.rm = TRUE)%>%
  ggplot(aes(arousal, getindaxis2))+
  geom_point()+
  ggpubr::stat_cor()+
  facet_grid(~arousal_3cat)
  # xlim(-,3.3)
...
View(clustering_new_pre_PCA_2_agg_by_val_sel_dcast2)
