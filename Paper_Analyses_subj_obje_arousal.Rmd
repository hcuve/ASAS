...
- what do we do with teh nested nature of teh data, one participant has multiple trials, if we don't accout for this, the cluster will treat data from different participants the same way.

If create cluster on a ndividual level- then the issue is different participants will end up with different clusters and clusters for one person are not translaable.
...
clust_dataset = clustering_agg[c(20,24,25)]
...
```{r}
clustering_agg$y_kmeans<- y_kmeans



?car::scatter3d
View(clustering_agg)
clustering_agg$y_kmeans<- as.factor(as.character(clustering_agg$y_kmeans))
...
cluster_lmers<- list()
clustering3$stimIAPS2<- as.factor(as.character(clustering3$stimIAPS2))

clustering3$ssid<- as.factor(as.character(clustering3$ssid))
clustering3
...
cluster_lmers$arousal <- lmer(arousal ~ cluster_interpreted + 
                               (1 | ssid)+
                               (1|stimIAPS2),
                             REML = FALSE,
                             data = clustering3)
...
emmeans::emmeans(cluster_lmers$arousal, pairwise~cluster_interpreted)

# valence
cluster_lmers$valence <- lmer(valence ~ cluster_interpreted + 
                               (1 | ssid)+
                               (1|stimIAPS2),
                             REML = FALSE,
                             data = clustering3)
...
emmeans::emmeans(cluster_lmers$valence, pairwise~cluster_interpreted)
