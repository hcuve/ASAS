...
timecourse_60bin_result_df_bp<- timecourse_60bin_result_df
...
mySpca <- spca(obj, ask=FALSE, type=1, scannf=FALSE)
?spca
mySpca <- spca(var.data, ask=FALSE, type=1, scannf=FALSE)
...
var.data$sid <- all.data3_forPCA$ssid
require("plyr")
require("abind")

daply(var.data, "sid", function(x) cov(x[, -ncol(x)]))
...
pca::cpc(var.data, method = "stepwise", k = 2)
...
# start by creating a cluster within self reports
?cut
clustering_new_pre_PCA$arousal_3cat <- cut(clustering_new_pre_PCA$arousal, 3, labels = c("low", "mid", "high"))


colnames(clustering_new_pre_PCA)

var.design_ar = as.factor(clustering_new_pre_PCA$arousal_3cat)

library(prettyGraphs)
library(InPosition)

View(clustering_new_pre_PCA)
clustering_new_pre_PCA[,6:8]


clustering_new_pre_PCA1<- subset(clustering_new_pre_PCA,
                                 !is.na(pup_basCor))

var.design_ar = as.factor(clustering_new_pre_PCA1$arousal_3cat)
 table(is.na(var.design_ar))
 
 clustering_new_pre_PCA2<- subset(clustering_new_pre_PCA1, !is.na(arousal_3cat))
 
 var.design_ar<- c(clustering_new_pre_PCA2$arousal_3cat)
 
 clustering_new_pre_PCA2$arousal_3cat <- cut(clustering_new_pre_PCA2$arousal, 2,
                                            labels = c("low", "high"))

# ExPosition:
 ?TInPosition::tepBADA.inference.battery
var.bada.ar_physio <- TInPosition::tepBADA.inference.battery(clustering_new_pre_PCA2[,6:8], 
                                                       scale = FALSE, 
                                                       center = FALSE, 
                                                       # DESIGN = factor( clustering_new_pre_PCA2$arousal_3cat ),
                                                        DESIGN = factor(var.design_ar ),
                                                       make_design_nominal = TRUE,
                                                       group.masses = NULL, 
                                                       weights = NULL, 
                                                       graphs = TRUE, 
...
var.bada.ar_physio[["Fixed.Data"]][["TExPosition.Data"]][["fii"]][,1]


clustering_new_pre_PCA2$test_pc1<-var.bada.ar_physio[["Fixed.Data"]][["TExPosition.Data"]][["fii"]][,1]
...
stimIapsval<- db_full4new_stim_screen_pupil_nopract %>%
  group_by(stimIAPS2)%>%
  summarise_at(c("ValenceMean", "ArousalMean"), mean, na.rm = TRUE)

clustering_new_pre_PCA3 <- left_join(clustering_new_pre_PCA2, stimIapsval)

clustering_new_pre_PCA3$arousal_3cat_ground<- cut(clustering_new_pre_PCA3$ArousalMean,3, labels = 
                                                    c("low", "mid", "high"))
...
var.design_ar_2<- factor(clustering_new_pre_PCA3$arousal_3cat_ground )

table(is.na( var.design_ar_2))
# sum(var.design_ar_2)
# TExPosition::tepBADA()
var.bada.ar_physio2 <- TInPosition::tepBADA.inference.battery(clustering_new_pre_PCA3[,6:8], 
                                                       scale = TRUE, 
                                                       center = TRUE, 
                                                       # DESIGN = factor( clustering_new_pre_PCA2$arousal_3cat ),
                                                        DESIGN = var.design_ar_2,
                                                       make_design_nominal = TRUE,
                                                       group.masses = NULL, 
                                                       weights = NULL, 
                                                       graphs = TRUE, 
...
clustering_new_pre_PCA3$arousal_3cat<- cut(clustering_new_pre_PCA3$valence,3, labels = 
                                                    c("neg", "mid", "pos"))
...
# sum(var.design_ar_2)
# TExPosition::tepBADA()
var.bada.vl_physio3 <- TInPosition::tepBADA.inference.battery(clustering_new_pre_PCA3[,6:8], 
                                                       scale = TRUE, 
                                                       center = TRUE, 
                                                       # DESIGN = factor( clustering_new_pre_PCA2$arousal_3cat ),
                                                        DESIGN = factor(clustering_new_pre_PCA3$arousal_3cat),
                                                       make_design_nominal = TRUE,
                                                       group.masses = NULL, 
                                                       weights = NULL, 
                                                       graphs = TRUE, 
...
clustering_new_pre_PCA3$vall_3_gr<- cut(clustering_new_pre_PCA3$ValenceMean,3, labels = 
                                                    c("neg", "mid", "pos"))
...
# sum(var.design_ar_2)
# TExPosition::tepBADA()
var.bada.vlgr_physio4 <- TInPosition::tepBADA.inference.battery(clustering_new_pre_PCA3[,6:8], 
                                                       scale = TRUE, 
                                                       center = TRUE, 
                                                       # DESIGN = factor( clustering_new_pre_PCA2$arousal_3cat ),
                                                        DESIGN = factor(clustering_new_pre_PCA3$vall_3_gr),
                                                       make_design_nominal = TRUE,
                                                       group.masses = NULL, 
                                                       weights = NULL, 
                                                       graphs = TRUE, 
...
res.mfa <- MFA(clustering_new_pre_PCA_2, 
               group = c(2, 1, 1, 1), 
               type = c("n", "s", "s", "s"),
               name.group = c("pp_stim","pupil","HR","SCR"),
               # num.group.sup = c(1,2),
               graph = TRUE)

colnames(clustering_new_pre_PCA3)

clustering_new_pre_PCA3$arousal_3cat<- cut(clustering_new_pre_PCA3$arousal,3, labels = 
                                                    c("low", "mid", "high"))
c<-FactoMineR::MFA(clustering_new_pre_PCA3[,c(1,14,6:8)], 
               group = c(2, 1, 1, 1), 
               type = c("n", "s", "s", "s"),
               name.group = c("pp_stim","pupil","HR","SCR"),
               num.group.sup = c(1,2),
...
?FactoMineR::MFA
FactoMineR::MFA(clustering_new_pre_PCA3[,c(1,4,6:8,9,10)], 
               group = c(2, 1, 1, 1,1,1), 
               type = c("n", "s", "s", "s", "s", "s"),
               name.group = c("pp_stim","pupil","HR","SCR", "arousal", "valence"),
               num.group.sup = c(1,2),
               graph = TRUE)

# valence and arousal are creatign a strign compoent one maybe vecause of thir correlatipn
# so below I am droping one to see if we have more coherence
# ?
FactoMineR::MFA(clustering_new_pre_PCA3[,c(1,4,6:8,9)], 
               group = c(2, 1, 1, 1,1), 
               type = c("n", "s", "s", "s", "s"),
               name.group = c("pp_stim","pupil","HR","SCR", "arousal"),
               num.group.sup = c(1,2),
               ind.sup = c(1,2),
               graph = TRUE)
...
# include 
colnames(clustering_new_pre_PCA3)
unique(clustering_new_pre_PCA3$stimDescription)

clustering_new_pre_PCA3$AR
clustering_new_pre_PCA3.1<- clustering_new_pre_PCA3[,c(1,4,6,7,8,9,10)]
res.mfa_bio_vs_self<- FactoMineR::MFA(clustering_new_pre_PCA3.1, 
               group = c(2, 3, 2), 
               type = c("n", "s", "s"),
               # "s", "s", "s"
...
```{r}
# use stim as indicator
colnames(clustering_new_pre_PCA3)

clustering_new_pre_PCA3$val_cut_iaps<- cut(clustering_new_pre_PCA3$ValenceMean, 3, labels = 
                                             c("neg", "mid", "pos"))


clustering_new_pre_PCA3$aro_cut_iaps<- cut(clustering_new_pre_PCA3$ArousalMean, 3, labels = 
                                             c("low", "mid", "high"))
clustering_new_pre_PCA3.1<- clustering_new_pre_PCA3[,c(1,5,22,23,6,7,8,9,10)]
colnames(clustering_new_pre_PCA3.1)

res.mfa_bio_vs_self2<- FactoMineR::MFA(clustering_new_pre_PCA3.1[,1:9], 
               group = c(4, 3, 2), 
               type = c("n", "s", "s"),
               # "s", "s", "s"
               name.group = c("pp_stim","auton","subj"),
...
# now z score within participant
clustering_new_pre_PCA3.1.scale<- clustering_new_pre_PCA3.1%>%
  group_by(ssid)%>%
  mutate_if(is.numeric, scale)


clustering_new_pre_PCA3.1.scale$stim_descr

clustering_new_pre_PCA3.1.scale

res.mfa_bio_vs_self2_scale <- FactoMineR::MFA(clustering_new_pre_PCA3.1.scale[,2:7], 
               group = c(1, 3, 2), 
               type = c("n", "s", "s"),
               # "s", "s", "s"
               name.group = c("pp_stim","auton","subj"),
...
dimdesc(res.mfa_bio_vs_self2_scale)
...
# validation
# -	Some idea, get p values for factorminer – dimensions, and correklations
# -	Stability of firt 2 dimensions by resampling



res.mfa_bio_vs_self2_k2<- FactoMineR::MFA(clustering_new_pre_PCA3.1[,1:9], 
               group = c(4, 3, 2), 
               type = c("n", "s", "s"),
              ind.sup = c(1),
              ncp = 2,
...
# randomly resample withouth replacement

?FactoMineR::simule()

clustering_new_pre_PCA3.1$stimDescription<- as.factor(clustering_new_pre_PCA3.1$stimDescription)


clustering_new_pre_PCA3.1$ssid_stim<- paste0(clustering_new_pre_PCA3.1$ssid,
                                             paste0(clustering_new_pre_PCA3.1$stimDescription))


clustering_new_pre_PCA3.1$ssid_stim<- as.factor(clustering_new_pre_PCA3.1$ssid_stim)

unique(clustering_new_pre_PCA3.1$ssid)

# this will simulkate 61 times (we can assum that 51 paticipants)

testsim$simul

# clustering_new_pre_PCA3.1$stimDescription

val_cutlabels<- clustering_new_pre_PCA3.1%>%
  group_by(stimDescription,val_cut_iaps) %>%
  summarise_if(is.numeric, mean, na.rm = TRUE)%>%
  select(c("stimDescription", "val_cut_iaps"))


testsim<- FactoMineR::simule(clustering_new_pre_PCA3.1[,c(2,5:9)], 1)
# then fit pca store the
testsim_withlabels <- left_join(testsim$simul, val_cutlabels)
View(testsim_withlabels)
# then do it again a 100 times

# correlation between variable and dimension


res.mfa_bio_vs_self2_k2[["quanti.var"]][["coord"]]

res.mfa_bio_vs_self2_k2$eig[1:2,]





# https://declaredesign.org/r/fabricatr/reference/resample_data.html


resample_data(clustered_survey, N = 5, ID_labels = "clusters")

unique(clustering_new_pre_PCA3.1$ssid)
clustering_new_pre_PCA3.1[,c(1,2,5:9)]


testres<- resample_data(clustering_new_pre_PCA3.1 [,c(1,2,5:9)], N = 51, ID_labels = "ssid")

# resample_data(clustering_new_pre_PCA3.1[,c(1,2,5:9)], N = 51, 
#               ID_labels = c("ssid","stimDescription")) this can throw errior fu to unbalances
...
# pca
resampled_temp
# create emopty dataframe
resamp_1st<- data.frame(matrix(NA, nrow = 1, ncol = 9))
colnames(resamp_1st)<- names(resampled_temp)

# create emoty  storage rame for eigenvakues
eigen_first<- as.data.frame(eugentest)
eigen_first[,]<- NA

# eigen_first<- as.data.frame(eigen_first)

# create empty storage for correlations
cor_first<- as.data.frame(cortest)
cor_first[,]<- NA
# cor_first<- as.data.frame(cor_first)


# now store group results/partials

res.mfa_bio_vs_self2_k2$group$correlation
partials_first <- as.data.frame(res.mfa_bio_vs_self2_k2[["partial.axes"]][["contrib"]])
partials_first[,]<- NA
partials_first
partials_first$sim_no<- NA
...
unique(resampled_temp_test$ssid)
nrow(resampled_temp_test)

 mean(resampled_temp_test$pup_basCor) #0.6710218
 
 table(resampled_temp_test$ssid)
 
 View(resampled_temp_test)
 
 clustering_new_pre_PCA3.1%>%
   subset(ssid == 1)
 
 
resampled_temp_test<- resampled_temp_test %>%
  group_by(ssid, stimDescription, val_cut_iaps)%>%
  mutate(dupe = n()>1)

table(resampled_temp_test$dupe)
 
 
 install.packages("fabricatr")
 ??fabricatr::resample_data
 View(resampled_temp_test)
 unique(resampled_temp_test$val_cut_iaps_unique)
 clustering_new_pre_PCA3.1$ssid<- as.character(clustering_new_pre_PCA3.1$ssid)

 table(resampled_temp_test$val_cut_iaps)
 
 
 for (i in 1:1000) {
  message(sprintf("$$$$$RUNING sim %i", i))
...
resamp_1st_N30_withrepl<- resamp_1st
eigen_first_N30_withrepl<- eigen_first
cor_first_N40_withrepl<- cor_first

partials_first_n30_withrepl<- cor_first
...
for (i in 1:1500) {
  message(sprintf("$$$$$RUNING sim %i", i))
  resampled_temp<-  resample_data(clustering_new_pre_PCA3.1 [,c(1,2,3, 5:9)], 
                                  N = 80, ID_labels = "ssid")
  resampled_temp$sim_no = i
...
# check if all particiants ocur equally in the old sample
table(resamp_1st_N30_withrepl$ssid)
table(resamp_1st$ssid)


resamp_1st_N80_withrepl<- resamp_1st
eigen_first_N80_withrepl<- eigen_first
cor_first_N80_withrepl<- cor_first

partials_first_n40_withrepl<- partials_first

resamp_1st_N80_withrepl
...
##################################################################
# resample hierarquically, use lower n for subject and valence
##################################################################


```{r}
View(resampled_temptest)
colnames(resamp_1st)
colnames(resampled_temp_test)

 # create emopty dataframe
resamp_1st<- data.frame(matrix(NA, nrow = 1, ncol = 13))
# cnames_resamp<- names(resampled_temp)
# colnames(resamp_1st)<- names(resampled_temp)
colnames(resamp_1st)<- cnames_resamp
colnames(resamp_1st)
# create emoty  storage rame for eigenvakues
eigen_first<- as.data.frame(eugentest)
eigen_first[,]<- NA
...
clustering_new_pre_PCA3.1$ssid<- as.character(clustering_new_pre_PCA3.1$ssid)
clustering_new_pre_PCA3.1$stimDescription<- as.character(clustering_new_pre_PCA3.1$stimDescription)
# having clusters as factors causes a subscript oput of bound error


# think we need to store groups

groups_first<-as.data.frame(  res.mfa_bio_vs_self2_k2$group$correlation)
# res.mfa_bio_vs_self2_k2$group$correlation

groups_first[,]<- NA
groups_first
groups_first$sim_no<- NA


clustering_new_pre_PCA3.1 [,c(1,2,3, 5:9)]

unique(substr(resampled_temp_test$stimDescription_unique,1,6))
unique(substr(resampled_temp_test$stimDescription_unique,7,30))

#################be careful to use the correct mfa.res_res. a few times i forgot to use the right one and got constants in the updated dataframes (e.g. partials)
for (i in 1:5000) {
  message(sprintf("$$$$$RUNING sim %i", i))
  resampled_temp<- fabricatr::resample_data(clustering_new_pre_PCA3.1 [,c(1,2,3, 5:9)], 
                                  N = c(51,51),
                                  ID_labels = c("ssid", "stimDescription"),
                                  unique_labels = TRUE)
  resampled_temp$sim_no = i
  resampled_temp$ssid_new <- as.factor(substr(resampled_temp$stimDescription_unique,1,6))
  resampled_temp$newstim <- substr(resampled_temp$stimDescription_unique,7,30)
  # resampled_temp$stimDescription<- as.factor(resampled_temp$stimDescription)
  
# run PCA on the resampled data
# ?list
    message(sprintf("$$$$$fitting PCA %i", i))
    mfa.res_res <- FactoMineR::MFA(resampled_temp[,c(12,2:8)], 
               group = c(3, 3, 2), 
               type = c("n", "s", "s"),
              # ind.sup = c(1),
              ncp = 2,
...
partials_temp<- as.data.frame(mfa.res_res$partial.axes$cor)
partials_temp$sim_no = i

# groups
group_temp<- as.data.frame(mfa.res_res$group$cos2)
group_temp$sim_no = i
...
cor_first<- bind_rows(cor_first, cortest)
partials_first<- bind_rows(partials_first, partials_temp)
groups_first<- bind_rows(groups_first, group_temp)
  message(sprintf("$$$$$completed iteration%i", i))
}
...
resamp_1st_1000boot_hier<- resamp_1st
eigen_first_1000boot_hier<-eigen_first
cor_first_100boot_hier<- cor_first

partials_first_100boot_hier<-partials_first
group_first_100boot_hier<-groups_first
...
# check component eigenvalues
# quantile(eigenvalue,.05, na.rm = TRUE)
View(eigen_first)

eigen_first$eigenvalue
...
eigen_first%>%
  ...
  mutate(perc_5 = quantile(eigenvalue,.025, na.rm = TRUE), 
         perc_95 = quantile(eigenvalue,.975, na.rm = TRUE)) %>% #create 95% to descrive interval around the observed estimate
  # mutate(probabcheck= sum(eigenvalue>=res.mfa_bio_vs_self2_k2$eig[1])/n())
  ggplot(aes(eigenvalue))+
  geom_histogram(alpha = .2)+
    geom_density()+
  # facet_grid(~component)+
    geom_errorbar(aes(xmin = min(perc_5), xmax=max(perc_95), y = 10), color = "gray30")+
  geom_point(x = res.mfa_bio_vs_self2_k2$eig[1],y = 10,size = 2, color = "red")+

  theme_classic()+
  geom_vline(xintercept = 1, linetype = "dashed")+
  ggtitle("distribution of boostraped eigen values for component  1 
          red dot = observed eigenvalue, boostrapped 95%CI")+
  annotate("text", x = 1.2, y = 20, label = "prob of obs eigen>= boot eigen = 51%")

table(eigen_first$eigenvalue>1)

t.testsample1<- eigen_first%>%
  mutate(component =as.factor(substr(rownames(eigen_first), 1,6)))%>% #retrieve names for components
  subset(!is.na(eigenvalue) & component == "comp 1" )

t.test(1.14, mu = mean(t.testsample1$eigenvalue))

# boostrap ratio
# proposed by abdi for factir scores
mean(t.testsample1$eigenvalue, na.rm = TRUE)/(sd(t.testsample1$eigenvalue,na.rm = TRUE))
...
```{r}
# check eigen vectors (correlations

# checpartials_first_n40_withrepl

# check groups

# binom.test(eigen_first$eigenvalue>1)



# calculate the diffeence betwee true eufgenvalye abd boostrap eigenvakue
# and check if it includes zero?

eigen_first%>%
  mutate(component =as.factor(substr(rownames(eigen_first), 1,6)))%>% #retrieve names for components
  subset(!is.na(eigenvalue) & component == "comp 1" )%>% #select only component 1 and remove nas I introduced in the dataframe
  mutate(eigen_d = (mean(eigenvalue))- res.mfa_bio_vs_self2_k2$eig[1])%>%
   mutate(eigen_d2 = eigenvalue- res.mfa_bio_vs_self2_k2$eig[1])%>%
  
  mutate(perc_5 = quantile(eigen_d2,.025, na.rm = TRUE), 
         perc_95 = quantile(eigen_d2,.975, na.rm = TRUE))%>% #create 95% to descrive interval around the observed estimate
  ggplot(aes(eigen_d2))+
  geom_histogram(alpha = .2)+
    geom_density()+
  # facet_grid(~component)+
    geom_errorbar(aes(xmin = min(perc_5), xmax=max(perc_95), y = 10), color = "gray30")+
  # geom_point(aes(x = eigen_d,y = 10,size = 2), color = "red")+

  theme_classic()+
    ggtitle("distribution of observed eigenvalue - boot eigenvalues for component  1")+
  annotate("text", x = 0, y = 20, label = "95%CI")
...
t.test_sample<- eigen_first%>%
  mutate(component =as.factor(substr(rownames(eigen_first), 1,6)))%>% #retrieve names for components
  subset(!is.na(eigenvalue) & component == "comp 1" )%>% #select only component 1 and remove nas I introduced in the dataframe
  mutate(eigen_d = (mean(eigenvalue))- res.mfa_bio_vs_self2_k2$eig[1])%>%
   mutate(eigen_d2 = eigenvalue- res.mfa_bio_vs_self2_k2$eig[1])
t.test(t.test_sample$eigen_d2)


# technially a t test is calculated as the differnce between current mean and pipulation mean devied by population variance, since we often don't have population variance we estimate it from the sample


# will this give us a t?
(res.mfa_bio_vs_self2_k2$eig[1]- (mean(t.test_sample$eigenvalue, na.rm = TRUE)))/ 
  (sd(t.test_sample$eigenvalue))

mean(t.test_sample$eigen_d2)/  (sd(t.test_sample$eigenvalue))



resamp_1st%>%
  subset(sim_no = 1)%>%
  subset(is.na(ssid) == FALSE)%>%
  subset(ssid ==  306)
...
fabricate(
  primary_schools = add_level(N = 3, 
                              var1 = runif(N, 0, 10)),
  students = link_levels(N = 30,
                          by=join(var1, rho = 0.5)))
...
quantile(eigen_first$eigenvalue,.95, na.rm = TRUE)
eigen_first%>%
  mutate(component =as.factor(substr(rownames(eigen_first), 1,6)))%>%
    subset(!is.na(eigenvalue) & component == "comp 2" )%>%
  mutate(perc_5 = quantile(eigenvalue,.025, na.rm = TRUE),
  ...
  geom_histogram(alpha = .2)+
    geom_density()+
   geom_errorbar(aes(xmin = min(perc_5), xmax=max(perc_95), y = 10), color = "gray30")+
  geom_point(x = res.mfa_bio_vs_self2_k2$eig[2],y = 10,size = 2, color = "red")+
  # geom_vline(aes(xintercept = perc_95), linetype = "dotted")+
  geom_errorbarh(aes(xmax = perc_95, xmin = perc_5, y = 10))+
  theme_classic()
  
  
```
  
  # first dimension is significant

```{r}

 # same thing as above with variance
# quantile(eigen_first$`percentage of variance`,.95, na.rm = TRUE)
eigen_first%>%
  mutate(component =as.factor(substr(rownames(eigen_first), 1,6)))%>%
  subset(!is.na(`percentage of variance`) & component == "comp 1" )%>%
  mutate(perc_5 = quantile(`percentage of variance`,.025, na.rm = TRUE),
         perc_95 = quantile(`percentage of variance`,.975, na.rm = TRUE))%>%
  ggplot(aes(`percentage of variance`))+
  geom_histogram(aes(y=..density..), alpha=0.5,
                 position="identity")+
  geom_density(alpha=.2) +
  geom_errorbarh(aes(xmax = perc_95, xmin = perc_5, y = .3), color = "gray30")+
  geom_point(x = res.mfa_bio_vs_self2_k2$eig[1,2],y = .3,size = 3, color = "red")+

    # geom_vline(aes(xintercept = perc_5), linetype = "dotted")+
  # geom_vline(aes(xintercept = perc_95), linetype = "dotted")+
  theme_classic()
...
```{r}
# let's go to variable correlations with dimenstions

cor_first %>%
  mutate(variable = as.factor(substr(rownames(cor_first), 1,6)))%>%
    subset(!is.na(Dim.1)) %>%
  # create interval per variable
  group_by(variable)%>%
   mutate(perc_5 = quantile(Dim.1,.025, na.rm = TRUE),
         perc_95 = quantile(Dim.1,.975, na.rm = TRUE))%>%
  # subset(variable == "arousa"|variable == "valenc")%>%
  # select(c("arousal", "valence"))%>%
  ggplot(aes(Dim.1)) +
  geom_histogram(aes(y=..density..), alpha=0.5,
                 position="identity")+
  geom_density(alpha=.2) +
   geom_errorbarh(aes(xmax = perc_95, xmin = perc_5, y =.6 ), color = "gray30")+
  facet_grid(~variable)+
...
cor_first %>%
  ...
  # create interval per variable
  group_by(variable)%>%
   mutate(perc_5 = quantile(Dim.2,.025, na.rm = TRUE),
         perc_95 = quantile(Dim.2,.975, na.rm = TRUE))%>%
  # subset(variable == "arousa"|variable == "valenc")%>%
  # select(c("arousal", "valence"))%>%
  ggplot(aes(Dim.2)) +
  geom_histogram(aes(y=..density..), alpha=0.5,
  ...
  facet_grid(~variable)+
  # geom_vline(xintercept = .5, color = "red", linetype = "dashed", alpha = .1, size = 1)+
   geom_vline(xintercept = 0, color = "red", linetype = "dashed", alpha = .1, size = 1)+
  theme_classic()
...
partials_first%>%
  mutate(dimension = as.factor(substr(rownames(partials_first), 1,4)),
         group = as.factor(substr(rownames(partials_first), 5,9)))%>%
    subset(!is.na(Dim.1)) %>%
  # create interval per variable
  ...
  #                position="identity")+
  geom_density(alpha=.2) +
   geom_errorbarh(aes(xmax = perc_95, xmin = perc_5, y =10 ), color = "gray30")+
  facet_grid(dimension~group)+
  # geom_vline(xintercept = .5, color = "red", linetype = "dashed", alpha = .1, size = 1)+
   # geom_vline(xintercept = -.5, color = "red", linetype = "dashed", alpha = .1, size = 1)+
  theme_classic()
  ylim(0,100)
...
# we sgoudl probably store cos2 rather than correlatiopn
# their cos2 values corresponding to their quality of representation on the factor map
groups_first%>%
  mutate(group = as.factor(substr(rownames(groups_first), 1,4)))%>%
    subset(!is.na(Dim.1)) %>%
  # create interval per variable
  ...
  # -perc_5,-perc_95, 
   ...
   mutate(perc_5 = quantile(value_dimenion,.025, na.rm = TRUE),
         perc_95 = quantile(value_dimenion,.975, na.rm = TRUE))%>%
...
Do we get the same patterns when loking at paticipants and stimuli searatelly

```{r}
# get_eigenvalue(res.mfa): Extract the eigenvalues/variances retained by each dimension (axis)
eig.val_bui_self_scl <- get_eigenvalue(res.mfa_bio_vs_self2_scale)
head(eig.val_bui_self_scl)

# The function fviz_eig() or fviz_screeplot() [factoextra package] can be used to draw the scree plot:

fviz_screeplot(res.mfa_bio_vs_self2_scale)


group_bio_self_scl <- get_mfa_var(res.mfa_bio_vs_self2_scale, "group")
group_bio_self_scl


# Coordinates of groups
options(scipen = 999)
head(group_bio_self_scl$coord)

#    Dim.1      Dim.2      Dim.3     Dim.4        Dim.5
# auton 0.5042850 0.98835343 0.85875961 0.4433362 0.0008990348
# subj  0.6790127 0.00720065 0.01629939 0.3014437 0.2019402875

head(group_bio_self_scl$cos2) 
# Cos2: quality of representation on the factore map

#        Dim.1         Dim.2        Dim.3      Dim.4           Dim.5
# auton 0.09726538 0.37362051551 0.2820651231 0.07517484 0.0000003091428
# subj  0.44230732 0.00004974068 0.0002548654 0.08717275 0.0391213886164

# Contributions to the  dimensions
head(group_bio_self_scl$contrib)

#          Dim.1      Dim.2     Dim.3   Dim.4      Dim.5
# auton 42.61692 99.2767194 98.137338 59.5258  0.4432251
# subj  57.38308  0.7232806  1.862662 40.4742 99.5567749


# draw a bar plot of groups contribution to the dimensions

# Contribution to the first dimension
fviz_contrib(res.mfa_bio_vs_self2_scale, "group", axes = 1)
# Contribution to the second dimension
fviz_contrib(res.mfa_bio_vs_self2_scale, "group", axes = 2)
# basically they are contibuting to separate dimensions


# is this improved if we dropping pupil?


# res.mfa_bio_vs_self2_scale_nopup<- FactoMineR::MFA(clustering_new_pre_PCA3.1.scale[,c(2, 4,6:7)], 
#                group = c(1, 1, 2), 
#                type = c("n", "s", "s"),
#                # "s", "s", "s"
#                name.group = c("pp_stim","auton","subj"),
...
# 
# # Contribution to the first dimension
# fviz_contrib(res.mfa_bio_vs_self2_scale_nopup, "group", axes = 1)
# # Contribution to the second dimension
# fviz_contrib(res.mfa_bio_vs_self2_scale_nopup, "group", axes = 2)
# basically they are contibuting to separate dimensions

#
# is this improved if we dropping pupil? almost but not really

# head(res.mfa_bio_vs_self2_scale_nopup$global.pca)

?dimdesc
probabchecl<- dimdesc(res.mfa_bio_vs_self2_scale, axes = 1:2, proba = .05)

```

quantitative variables
```{r}
quanti.var_bio_vs_self2_scale <- get_mfa_var(res.mfa_bio_vs_self2_scale, "quanti.var")
quanti.var_bio_vs_self2_scale
...
res.mfa_bio_vs_self2_scale
fviz_mfa_var(res.mfa_bio_vs_self2_scale, 
             "quanti.var", 
             palette = "jco", 
             col.var.sup = "violet", 
             repel = TRUE,
...
fviz_mfa_var(res.mfa_bio_vs_self2_scale, "quanti.var", col.var = "contrib", 
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"), 
...
fviz_contrib(res.mfa_bio_vs_self2_scale, choice = "quanti.var", axes = 1, top = 20,
             palette = "jco")

fviz_contrib(res.mfa_bio_vs_self2_scale, choice = "quanti.var", axes = 2, top = 20,
             palette = "jco")

```
Graph of individuals  = rows
```{r}
fviz_mfa_ind(res.mfa_bio_vs_self2_scale, col.ind = "cos2", 
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE)
...
clustering_new_pre_PCA3.1.scale$stimDescription
??fviz_mfa_ind
fviz_mfa_ind(res.mfa_bio_vs_self2, 
             habillage = "aro_cut_iaps", # color by groups 
...
```{r}
fviz_ellipses(res.mfa_bio_vs_self2, c("val_cut_iaps"), repel = TRUE)
...
fviz_mfa_ind(res.mfa_bio_vs_self2, partial = "all") 
...
??HMFA
```{r}
H_n<- clustering_new_pre_PCA3.1.scale%>%
  group_by(ssid)%>%
  mutate(n = n())%>%
...
clustering_new_pre_PCA3.1.scale_agg_bystim<- clustering_new_pre_PCA3.1%>%
  group_by(stimDescription,val_cut_iaps)%>%
  summarise_if(is.numeric, mean, na.rm = TRUE)

clustering_new_pre_PCA3.1.scale_agg_bystim_scaleafteragg<- clustering_new_pre_PCA3.1.scale_agg_bystim%>%
  ungroup()%>%
  mutate_if(is.numeric, scale)

clustering_new_pre_PCA3.1.scale_agg_bystim_scaleafteragg$val_cut_iaps

res.mfa_bio_vs_self2_scale_aggbystim<- FactoMineR::MFA(clustering_new_pre_PCA3.1.scale_agg_bystim, 
               group = c(1, 3, 2), 
               type = c("n", "s", "s"),
               # "s", "s", "s"
               name.group = c("stim","auton","subj"),
               num.group.sup = c(1),
               # ind.sup = c(1,2),
               graph = TRUE)
...
```{r}
res.mfa_bio_vs_self2_scale_afteraggbystim<- FactoMineR::MFA(clustering_new_pre_PCA3.1.scale_agg_bystim_scaleafteragg, 
               group = c(1, 3, 2), 
               type = c("n", "s", "s"),
               # "s", "s", "s"
               name.group = c("stim","auton","subj"),
...
one approach for significance is to create random shuffling that destroys the relational aspect of the data,
e.g. by resshugling the values betwen variabels per participant.
e.g. go to participant one, resgule the order of puopil values for  trials, fo th scr fo the same and so on

```{r}

clustering_new_pre_PCA3.1_copy<- clustering_new_pre_PCA3.1
colnames(clustering_new_pre_PCA3.1)

?sample

# this is slow so need to figure  a better way to to this but for now
clustering_new_pre_PCA3.1_copy %>%
  subset(ssid == 304 &val_cut_iaps == "mid" & aro_cut_iaps == "low" )%>%
  group_by(ssid)%>%
  transform(pup_basCor = sample(pup_basCor, replace = TRUE),
            Bio_Mean_HR_dif = sample(Bio_Mean_HR_dif, replace = TRUE),
            log_BIO_CDA.PhasicMax = sample(log_BIO_CDA.PhasicMax, replace = TRUE),
            arousal = sample(arousal, replace = F),
            valence = sample(valence, replace = F))
...
colnames(resampled_temp_reshuf)
 # create emopty dataframe
resamp_1st_reshuf<- data.frame(matrix(NA, nrow = 1, ncol = 9))
cnames_resamp_reshufl<- names(resampled_temp_reshuf)

colnames(resamp_1st_reshuf)<- cnames_resamp_reshufl
colnames(resamp_1st_reshuf)
# create emoty  storage rame for eigenvakues
eigen_first_reshuf<- as.data.frame(eugentest)
eigen_first_reshuf[,]<- NA

# eigen_first_reshuf<- as.data.frame(eigen_first_reshuf)

# create empty storage for correlations
cor_first_reshuf<- as.data.frame(cortest)
cor_first_reshuf[,]<- NA
# cor_first_reshuf<- as.data.frame(cor_first_reshuf)


# now store group results/partials

# res.mfa_bio_vs_self2_k2$group$correlation
partials_first_reshuf <- as.data.frame(res.mfa_bio_vs_self2_k2[["partial.axes"]][["contrib"]])
partials_first_reshuf[,]<- NA
partials_first_reshuf
partials_first_reshuf$sim_no<- NA
...
groups_first_reshuf<-as.data.frame(  res.mfa_bio_vs_self2_k2$group$correlation)
# res.mfa_bio_vs_self2_k2$group$correlation

groups_first_reshuf[,]<- NA
groups_first_reshuf
groups_first_reshuf$sim_no<- NA
...
# changed below to replace treue - shuflle with replacement
  # https://stats.stackexchange.com/questions/104040/resampling-simulation-methods-monte-carlo-bootstrapping-jackknifing-cross?rq=1
...
resampled_temp_reshuf <-    clustering_new_pre_PCA3.1_copy[,c(1,2,3, 5:9)] %>%
  group_by(ssid)%>%
  transform(pup_basCor = 
              sample(pup_basCor, replace = TRUE),
            Bio_Mean_HR_dif = sample(Bio_Mean_HR_dif, replace = TRUE),
            log_BIO_CDA.PhasicMax = sample(log_BIO_CDA.PhasicMax, replace = TRUE),
            arousal = sample(arousal, replace = TRUE),
            valence = sample(valence, replace = TRUE))
  
  resampled_temp_reshuf$sim_no = i
  # resampled_temp_reshuf$ssid_new <- as.factor(substr(resampled_temp_reshuf$stimDescription_unique,1,6))
  # resampled_temp_reshuf$newstim <- substr(resampled_temp_reshuf$stimDescription_unique,7,30)
  # resampled_temp_reshuf$stimDescription<- as.factor(resampled_temp_reshuf$stimDescription)
  
# run PCA on the resampled data
# ?list
    message(sprintf("$$$$$fitting PCA %i", i))
    mfa.res_reshuf <- FactoMineR::MFA(resampled_temp_reshuf[,c(1:8)], 
               group = c(3, 3, 2), 
               type = c("n", "s", "s"),
              # ind.sup = c(1),
              ncp = 2,
...
# store the eigen values
message(sprintf("$$$$$storing PCA %i", i))
eugentest_reshufl<- as.data.frame(mfa.res_reshuf$eig[1:2,])
eugentest_reshufl$sim_no<- i

cortest_reshufl<- as.data.frame(mfa.res_reshuf$quanti.var$cor)
cortest_reshufl$sim_no = i

partials_temp_reshuf<- as.data.frame(mfa.res_reshuf$partial.axes$cor)
partials_temp_reshuf$sim_no = i

# groups
group_temp_reshuf<- as.data.frame(mfa.res_reshuf$group$cos2)
group_temp_reshuf$sim_no = i


# update the initial d=frames
  message(sprintf("$$$$$upating dataframes %i", i))
  resamp_1st_reshuf<- (bind_rows(resamp_1st_reshuf, resampled_temp_reshuf))
eigen_first_reshuf<- bind_rows(eigen_first_reshuf, eugentest_reshufl)
cor_first_reshuf<- bind_rows(cor_first_reshuf, cortest_reshufl)
partials_first_reshuf<- bind_rows(partials_first_reshuf, partials_temp_reshuf)
groups_first_reshuf<- bind_rows(groups_first_reshuf, group_temp_reshuf)
  message(sprintf("$$$$$completed iteration%i", i))
}
...
visualise resshufled null data

```{r}


eigen_first_reshuf%>%
  mutate(component =as.factor(substr(rownames(eigen_first_reshuf), 1,6)))%>% #retrieve names for components
  subset(!is.na(eigenvalue) & component == "comp 1" )%>% #select only component 1 and remove nas I introduced in the dataframe
  mutate(perc_5 = quantile(eigenvalue,.025, na.rm = TRUE), 
         perc_95 = quantile(eigenvalue,.975, na.rm = TRUE)) %>% #create 95% to descrive interval around the observed estimate
  # mutate(probabcheck= sum(eigenvalue>=res.mfa_bio_vs_self2_k2$eig[1])/n())
  ggplot(aes(eigenvalue))+
  geom_histogram(bins = 500, alpha = .2)+
...
t.testsample1_reshudle<- eigen_first_reshuf%>%
  mutate(component =as.factor(substr(rownames(eigen_first), 1,6)))%>% #retrieve names for components
  subset(!is.na(eigenvalue) & component == "comp 1" )


t.testsample1_reshudle$eigen_extreme <- t.testsample1_reshudle$eigenvalue>= res.mfa_bio_vs_self2_k2$eig[1]
table(t.testsample1_reshudle$eigen_extreme)

?binom.test
binom.test(c(0,1000))

# let's do component 2


t.testsample1_reshudledim2<- eigen_first_reshuf%>%
  mutate(component =as.factor(substr(rownames(eigen_first), 1,6)))%>% #retrieve names for components
  subset(!is.na(eigenvalue) & component == "comp 2" )


t.testsample1_reshudledim2$eigen_extreme <- t.testsample1_reshudledim2$eigenvalue>= res.mfa_bio_vs_self2_k2$eig[2]
table(t.testsample1_reshudle$eigen_extreme)
...
eigen_first_reshuf%>%
  mutate(component =as.factor(substr(rownames(eigen_first_reshuf), 1,6)))%>% #retrieve names for components
  subset(!is.na(eigenvalue) & component == "comp 2" )%>% #select only component 1 and remove nas I introduced in the dataframe
  mutate(perc_5 = quantile(eigenvalue,.025, na.rm = TRUE), 
         perc_95 = quantile(eigenvalue,.975, na.rm = TRUE)) %>% #create 95% to descrive interval around the observed estimate
  # mutate(probabcheck= sum(eigenvalue>=res.mfa_bio_vs_self2_k2$eig[1])/n())
  ggplot(aes(eigenvalue))+
...
```{r}
cor_first_reshuf %>%
  mutate(variable = as.factor(substr(rownames(cor_first_reshuf), 1,6)))%>%
    subset(!is.na(Dim.1)) %>%
  # create interval per variable
  group_by(variable)%>%
   mutate(perc_5 = quantile(Dim.1,.025, na.rm = TRUE),
...
cor_first_reshuf %>%
  ...
  group_by(variable)%>%
   mutate(perc_5 = quantile(Dim.1,.025, na.rm = TRUE),
         perc_95 = quantile(Dim.1,.975, na.rm = TRUE))%>%
  subset(variable == "valenc")%>%
  mutate(valenc_cor_extreme_d1 = Dim.1>= abs(mfa.res_reshuf$quanti.var$cor[5,1]),
         valenc_cor_extreme_d2 = Dim.2>= abs(mfa.res_reshuf$quanti.var$cor[5,2]))%>%
  mutate(prob_extree_vl_d1 = (sum(valenc_cor_extreme_d1)/n()),
         prob_extree_val_d2 = (sum(valenc_cor_extreme_d2)/n()))
# p 0.019	0.532
...
cor_first_reshuf %>%
  ...
  group_by(variable)%>%
   mutate(perc_5 = quantile(Dim.1,.025, na.rm = TRUE),
         perc_95 = quantile(Dim.1,.975, na.rm = TRUE))%>%
  subset(variable == "arousa")%>%
  mutate(arous_cor_extreme_d1 = Dim.1>= abs(mfa.res_reshuf$quanti.var$cor[4,1]),
         arous_cor_extreme_d2 = Dim.2>= abs(mfa.res_reshuf$quanti.var$cor[4,2]))%>%
  mutate(prob_extree_vl_d1 = (sum(arous_cor_extreme_d1)/n()),
         prob_extree_val_d2 = (sum(arous_cor_extreme_d2)/n()))
#  p 0.532; 0.019
...
cor_first_reshuf %>%
  ...
  group_by(variable)%>%
   mutate(perc_5 = quantile(Dim.1,.025, na.rm = TRUE),
         perc_95 = quantile(Dim.1,.975, na.rm = TRUE))%>%
  subset(variable == "pup_ba")%>%
  mutate(pup_cor_extreme_d1 = Dim.1>= abs(mfa.res_reshuf$quanti.var$cor[1,1]),
         pup_cor_extreme_d2 = Dim.2>= abs(mfa.res_reshuf$quanti.var$cor[1,2]))%>%
  mutate(prob_extree_puup_d1 = (sum(pup_cor_extreme_d1))/n(),
         prob_extree_puup_d2 = (sum(pup_cor_extreme_d2))/n())
...
cor_first_reshuf %>%
  ...
  group_by(variable)%>%
   mutate(perc_5 = quantile(Dim.1,.025, na.rm = TRUE),
         perc_95 = quantile(Dim.1,.975, na.rm = TRUE))%>%
  subset(variable == "Bio_Me")%>%
  mutate(hr_cor_extreme_d1 = Dim.1>= abs(mfa.res_reshuf$quanti.var$cor[2,1]),
         hr_cor_extreme_d2 = Dim.2>= abs(mfa.res_reshuf$quanti.var$cor[2,2]))%>%
  mutate(prob_extree_hr_d1 = (sum(hr_cor_extreme_d1))/n(),
         prob_extree_hr_d2 = (sum(hr_cor_extreme_d2))/n())
# p< .016 d1 p> ,05 d2
...
cor_first_reshuf %>%
  ...
  group_by(variable)%>%
   mutate(perc_5 = quantile(Dim.1,.025, na.rm = TRUE),
         perc_95 = quantile(Dim.1,.975, na.rm = TRUE))%>%
  subset(variable == "log_BI")%>%
  mutate(scr_cor_extreme_Dim.1 = Dim.1>= abs(mfa.res_reshuf$quanti.var$cor[3,1]),
         scr_cor_extreme_Dim.2 = Dim.2>= abs(mfa.res_reshuf$quanti.var$cor[3,2]))%>%
  mutate(prob_extree_scr_dim1 = (sum(scr_cor_extreme_Dim.1))/n(),
         prob_extree_scr_dim2 = (sum(scr_cor_extreme_Dim.2))/n())

# cor nor significant in any dimension p > p05
...
partials_first_reshuf%>%
  mutate(dimension = as.factor(substr(rownames(partials_first_reshuf), 1,4)),
         group = as.factor(substr(rownames(partials_first_reshuf), 5,9)))%>%
    subset(!is.na(Dim.1)) %>%
  # create interval per variable
  group_by(dimension, group)%>%
   mutate(perc_5 = quantile(Dim.1,.025, na.rm = TRUE),
...
partials_first_reshuf %>%
  mutate(variable = as.factor(substr(rownames(partials_first_reshuf), 1,8))) %>%
    subset(!is.na(Dim.1)) %>%
  # create interval per variable
  group_by(variable)%>%
...
groups_first_reshuf<- groups_first_reshuf%>%
  mutate(group = as.factor(substr(rownames(groups_first_reshuf), 1,4)))

group_cordf<- as.data.frame(res.mfa_bio_vs_self2_k2$group$correlation)
group_cordf$group<-as.factor(substr(rownames(group_cordf), 1,4)) 

groups_first_reshuf<- left_join(groups_first_reshuf, group_cordf, by = "group")
...
View(groups_first_reshuf)
groups_first_reshuf%>%
  gather(dimension, value_dimenion, -group, -sim_no, -Dim.1.y, -Dim.2.y) %>%
  # -perc_5,-perc_95, 
   group_by(group,dimension)%>%
   mutate(perc_5 = quantile(value_dimenion,.025, na.rm = TRUE),
         perc_95 = quantile(value_dimenion,.975, na.rm = TRUE))%>%
  
  ggplot(aes(value_dimenion)) +
  geom_histogram(bins = 500)+
  geom_vline(aes(xintercept = Dim.1.y), linetype = "dashed", color = "red")+
   geom_vline(aes(xintercept = Dim.2.y),  linetype = "dashed", color = "blue")+
  # geom_histogram(aes(y=..density..), alpha=0.5,
  #                position="identity")+
  geom_density(alpha=.2) +
  facet_grid(dimension~group)+
...
```{r}
clustering_new_pre_PCA3.1.scale_agg_bysid<- clustering_new_pre_PCA3.1%>%
  group_by(ssid)%>%
  summarise_if(is.numeric, mean, na.rm = TRUE)



clustering_new_pre_PCA3.1.scale_aft_agg_bysid<- clustering_new_pre_PCA3.1.scale_agg_bysid%>%
  mutate_if(is.numeric, scale)

res.mfa_agg_bysubj <- FactoMineR::MFA(clustering_new_pre_PCA3.1.scale_agg_bysid, 
               group = c(1, 3, 2), 
               type = c("n", "s", "s"),
               # "s", "s", "s"
               name.group = c("sid","auton","subj"),
               num.group.sup = c(1),
               # ind.sup = c(1,2),
               graph = TRUE)
...
boostrap subjct results


```{r}

factoextra::fviz_eig(res.mfa_agg_bysubj)
factoextra::fviz_mfa_group(res.mfa_agg_bysubj)

res.mfa_bio_vs_self2_k2$eig
res.mfa_agg_bysubj$eig
res.mfa_agg_bysubj$quanti.var$cor
res.mfa_agg_bysubj$group$cos2
...
```{r}
colnames(resampled_temp_bysubj)
resamp_1st_bysubj<- data.frame(matrix(NA, nrow = 1, ncol = 8))

# cnames_resamp_subj<- names(resampled_temp_bysubj)
colnames(resamp_1st_bysubj)<- cnames_resamp_subj
colnames(resamp_1st_bysubj)
# create emoty  storage rame for eigenvakues
eigen_first_bysubj<- as.data.frame(eugentest)
eigen_first_bysubj[,]<- NA

# eigen_first<- as.data.frame(eigen_first)

# create empty storage for correlations
cor_first_bysubj<- as.data.frame(cortest)
cor_first_bysubj[,]<- NA
# cor_first<- as.data.frame(cor_first)
...
# res.mfa_bio_vs_self2_k2$group$correlation
partials_first_bysubj <- as.data.frame(res.mfa_bio_vs_self2_k2[["partial.axes"]][["contrib"]])
partials_first_bysubj[,]<- NA
partials_first_bysubj
partials_first_bysubj$sim_no<- NA


# hierarquical clustering
clustering_new_pre_PCA3.1.scale_agg_bysid$ssid<- as.character(clustering_new_pre_PCA3.1.scale_agg_bysid$ssid)
# clustering_new_pre_PCA3.1.scale_agg_bysid$stimDescription<- as.character(clustering_new_pre_PCA3.1.scale_agg_bysid$stimDescription)


# think we need to store groups

groups_first_bysubj<-as.data.frame(  res.mfa_bio_vs_self2_k2$group$correlation)
# res.mfa_bio_vs_self2_k2$group$correlation

groups_first_bysubj[,]<- NA
groups_first_bysubj
groups_first_bysubj$sim_no<- NA



#################be careful to use the correct mfa.res_res. a few times i forgot to use the right one and got constants in the updated dataframes (e.g. partials)
colnames(clustering_new_pre_PCA3.1.scale_agg_bysid)

for (i in 1:1000) {
  message(sprintf("$$$$$RUNING sim %i", i))
  
  resampled_temp_bysubj<- fabricatr::resample_data(clustering_new_pre_PCA3.1.scale_agg_bysid, 
                                  N = c(51),
                                  ID_labels = c("ssid"),
                                  unique_labels = TRUE)
  # View(resampled_temp_bysubj)
  resampled_temp_bysubj$sim_no = i
  # resampled_temp_bysubj$ssid_new <- as.factor(substr(resampled_temp_bysubj$stimDescription_unique,1,6))
  # resampled_temp_bysubj$newstim <- substr(resampled_temp_bysubj$stimDescription_unique,7,30)
  # resampled_temp_bysubj$stimDescription<- as.factor(resampled_temp_bysubj$stimDescription)
  
# run PCA on the resampled data
  colnames(resampled_temp_bysubj)
    # message(sprintf("$$$$$fitting PCA %i", i))
    mfa.res_bysubj <- FactoMineR::MFA(resampled_temp_bysubj[,1:6], 
               group = c(1, 3, 2), 
               type = c("n", "s", "s"),
              # ind.sup = c(1),
              ncp = 2,
...
# store the eigen values
message(sprintf("$$$$$storing PCA %i", i))
eugentest_bysubj<- as.data.frame(mfa.res_bysubj$eig[1:2,])
eugentest_bysubj$sim_no<- i

cortest_bysubj<- as.data.frame(mfa.res_bysubj$quanti.var$cor)
cortest_bysubj$sim_no = i

partials_temp_bysubj<- as.data.frame(mfa.res_bysubj$partial.axes$cor)
partials_temp_bysubj$sim_no = i

# groups
group_temp_bysubj<- as.data.frame(mfa.res_bysubj$group$cos2)
group_temp_bysubj$sim_no = i


# update the initial d=frames
  message(sprintf("$$$$$upating dataframes %i", i))
  resamp_1st_bysubj<- (bind_rows(resamp_1st_bysubj, resampled_temp_bysubj))
eigen_first_bysubj<- bind_rows(eigen_first_bysubj, eugentest_bysubj)
cor_first_bysubj<- bind_rows(cor_first_bysubj, cortest_bysubj)
partials_first_bysubj<- bind_rows(partials_first_bysubj, partials_temp_bysubj)
groups_first_bysubj<- bind_rows(groups_first_bysubj, group_temp_bysubj)
  message(sprintf("$$$$$completed iteration%i", i))
}

resamp_1st_bysubj
View(resamp_1st_bysubj)
...
visualise boostrap CIs for agg by subjec



```{r}
eigen_first_bysubj%>%
  mutate(component =as.factor(substr(rownames(eigen_first_bysubj), 1,6)))%>% #retrieve names for components
  subset(!is.na(eigenvalue) & component == "comp 1" )%>% #select only component 1 and remove nas I introduced in the dataframe
  mutate(perc_5 = quantile(eigenvalue,.025, na.rm = TRUE), 
         perc_95 = quantile(eigenvalue,.975, na.rm = TRUE)) %>% #create 95% to descrive interval around the observed estimate
  # mutate(probabcheck= sum(eigenvalue>=res.mfa_bio_vs_self2_k2$eig[1])/n())
  ...
  # facet_grid(~component)+
    geom_errorbar(aes(xmin = min(perc_5), xmax=max(perc_95), y = 10), color = "gray30")+
  geom_point(x = res.mfa_agg_bysubj$eig[1],y = 10,size = 2, color = "red")+
...
eigen_first_bysubj%>%
  ...
  # facet_grid(~component)+
    geom_errorbar(aes(xmin = min(perc_5), xmax=max(perc_95), y = 10), color = "gray30")+
  geom_point(x = res.mfa_agg_bysubj$eig[2],y = 10,size = 2, color = "red")+
...
variables and dimension corelations
```{r}
cor_first_bysubj %>%
  mutate(variable = as.factor(substr(rownames(cor_first_bysubj), 1,6)))%>%
    subset(!is.na(Dim.1)) %>%
  # create interval per variable
  group_by(variable)%>%
   mutate(perc_5 = quantile(Dim.1,.025, na.rm = TRUE),
  ...
  geom_density(alpha=.2) +
   geom_errorbarh(aes(xmax = perc_95, xmin = perc_5, y =.6 ), color = "gray30")+
  facet_grid(~variable)+
  # geom_point(xlim = res.mfa_agg_bysubj$quanti.var$cor, y = 4, color = "red", size = 2)+
  # geom_vline(xintercept = .5, color = "red", linetype = "dashed", alpha = .1, size = 1)+
  #  geom_vline(xintercept = -.5, color = "red", linetype = "dashed", alpha = .1, size = 1)+
  theme_classic()
...
```{r}
colnames(resamp_1st_bysubj_reshuf)
 # create emopty dataframe
resamp_1st_bysubj_reshuf<- data.frame(matrix(NA, nrow = 1, ncol = 7))
cnames_resamp_bysubj_reshuf<- names(resampled_temp_bysubj_reshuf)

colnames(resamp_1st_bysubj_reshuf)<- cnames_resamp_bysubj_reshuf
colnames(resamp_1st_bysubj_reshuf)
# create emoty  storage rame for eigenvakues
eigen_first_bysubj_reshuf<- as.data.frame(eugentest)
eigen_first_bysubj_reshuf[,]<- NA

# eigen_first_bysubj_reshuf<- as.data.frame(eigen_first_bysubj_reshuf)

# create empty storage for correlations
cor_first_bysubj_reshuf<- as.data.frame(cortest)
cor_first_bysubj_reshuf[,]<- NA
# cor_first_bysubj_reshuf<- as.data.frame(cor_first_bysubj_reshuf)


# now store group results/partials

# res.mfa_bio_vs_self2_k2$group$correlation
partials_first_bysubj_reshuf <- as.data.frame(res.mfa_bio_vs_self2_k2[["partial.axes"]][["contrib"]])
partials_first_bysubj_reshuf[,]<- NA
partials_first_bysubj_reshuf
partials_first_bysubj_reshuf$sim_no<- NA
...
groups_first_bysubj_reshuf<-as.data.frame(  res.mfa_bio_vs_self2_k2$group$correlation)
# res.mfa_bio_vs_self2_k2$group$correlation

groups_first_bysubj_reshuf[,]<- NA
groups_first_bysubj_reshuf
groups_first_bysubj_reshuf$sim_no<- NA
...
for (i in 1:1000) {
  ...
  # https://stats.stackexchange.com/questions/104040/resampling-simulation-methods-monte-carlo-bootstrapping-jackknifing-cross?rq=1
  # See the question on permutation vs bootstrapping - "The permutation test is best for testing hypotheses and bootstrapping is best for estimating confidence intervals".
resampled_temp_bysubj_reshuf <- clustering_new_pre_PCA3.1.scale_agg_bysid %>%
  # group_by(ssid)%>%
  transform(pup_basCor = 
              sample(pup_basCor, replace = TRUE),
            Bio_Mean_HR_dif = sample(Bio_Mean_HR_dif, replace = TRUE),
  ...
  resampled_temp_bysubj_reshuf$sim_no = i

# run PCA on the resampled data
# ?list
  colnames(resampled_temp_bysubj_reshuf)
    message(sprintf("$$$$$fitting PCA %i", i))
    mfa.res_bysubj_reshuf <- FactoMineR::MFA(resampled_temp_bysubj_reshuf[,c(1:6)], 
               group = c(1, 3, 2), 
               type = c("n", "s", "s"),
              # ind.sup = c(1),
              ncp = 2,
...
# store the eigen values
message(sprintf("$$$$$storing PCA %i", i))
eugentest_bysubj_reshufl<- as.data.frame(mfa.res_bysubj_reshuf$eig[1:2,])
eugentest_bysubj_reshufl$sim_no<- i

cortest_bysubj_reshufl<- as.data.frame(mfa.res_bysubj_reshuf$quanti.var$cor)
cortest_bysubj_reshufl$sim_no = i

partials_temp_bysubj_reshuf<- as.data.frame(mfa.res_bysubj_reshuf$partial.axes$cor)
partials_temp_bysubj_reshuf$sim_no = i

# groups
group_temp_bysubj_reshuf<- as.data.frame(mfa.res_bysubj_reshuf$group$cos2)
group_temp_bysubj_reshuf$sim_no = i


# update the initial d=frames
  message(sprintf("$$$$$upating dataframes %i", i))
  resamp_1st_bysubj_reshuf<- (bind_rows(resamp_1st_bysubj_reshuf, resampled_temp_bysubj_reshuf))
eigen_first_bysubj_reshuf<- bind_rows(eigen_first_bysubj_reshuf, eugentest_bysubj_reshufl)
cor_first_bysubj_reshuf<- bind_rows(cor_first_bysubj_reshuf, cortest_bysubj_reshufl)
partials_first_bysubj_reshuf<- bind_rows(partials_first_bysubj_reshuf, partials_temp_bysubj_reshuf)
groups_first_bysubj_reshuf<- bind_rows(groups_first_bysubj_reshuf, group_temp_bysubj_reshuf)
  message(sprintf("$$$$$completed iteration%i", i))
}

save.image("subjobjPCAfeb9.RData")
...
eigen_first_bysubj_reshuf%>%
  mutate(component =as.factor(substr(rownames(eigen_first_reshuf), 1,6)))%>% #retrieve names for components
  subset(!is.na(eigenvalue) & component == "comp 1" )%>% #select only component 1 and remove nas I introduced in the
  ggplot(aes(eigenvalue))+
  geom_histogram(bins = 500, alpha = .2)+
    geom_density(alpha = .3)+
  # facet_grid(~component)+
  ...
  theme_classic()+
  geom_vline(xintercept = res.mfa_agg_bysubj$eig[1], linetype = "dashed")
...
eigen_first_bysubj_reshuf%>%
  mutate(component =as.factor(substr(rownames(eigen_first_reshuf), 1,6)))%>% #retrieve names for components
  subset(!is.na(eigenvalue) & component == "comp 2" )%>% #select only component 1 and remove nas I introduced in the
  ggplot(aes(eigenvalue))+
  geom_histogram(bins = 500, alpha = .2)+
    geom_density(alpha = .3)+
  # facet_grid(~component)+
  ...
  theme_classic()+
  geom_vline(xintercept = res.mfa_agg_bysubj$eig[2], linetype = "dashed")
...
eigen_first_bysubj_reshuf%>%
  mutate(component =as.factor(substr(rownames(eigen_first), 1,6)))%>% #retrieve names for components
  subset(!is.na(eigenvalue) & component == "comp 1" )%>%
  mutate(extreme_eigen_count = eigenvalue>= res.mfa_agg_bysubj$eig[1])%>%
  mutate(extreme_eigen_count_prob = (sum(extreme_eigen_count))/n())

# p value 0.702 as we can see in the graph
...
cor_first_bysubj_reshuf %>%
  mutate(variable = as.factor(substr(rownames(cor_first_bysubj_reshuf), 1,6)))%>%
    subset(!is.na(Dim.1)) %>%
  group_by(variable)%>%
  ggplot(aes(Dim.1)) +
  geom_histogram(aes(y=..density..), alpha=0.5,
...
cor_first_bysubj_reshuf %>%
  ...
  group_by(variable)%>%
   ...
   #       perc_95 = quantile(Dim.1,.975, na.rm = TRUE))%>%
  subset(variable == "Bio_Me")%>%
  mutate(hr_cor_extreme_d1 = abs(Dim.1)>= abs(mfa.res_bysubj_reshuf$quanti.var$cor[2,1]),
         hr_cor_extreme_d2 = abs(Dim.2)>= abs(mfa.res_bysubj_reshuf$quanti.var$cor[2,2]))%>%
  mutate(prob_extree_hr_d1 = (sum(hr_cor_extreme_d1))/n(),
         prob_extree_hr_d2 = (sum(hr_cor_extreme_d2))/n())
...
```{r}
# res.mfa_agg_bysubj$partial.axes$cor
partials_first_bysubj_reshuf %>%
  mutate(dimension = as.factor(substr(rownames(partials_first_bysubj_reshuf), 1,4)),
         group = as.factor(substr(rownames(partials_first_reshuf), 5,9)))%>%
    subset(!is.na(Dim.1)) %>%
  group_by(dimension, group)%>%
...
Agg within participant by neg pos etc (using normative data)
With and without selfreports


start with bootsrap to estimate CIS
```{r}

clustering_new_pre_PCA3_val_vat_agg_scale # this is the one I need
colnames(clustering_new_pre_PCA3_val_vat_agg_scale)

res.mfa_agg_bysubj_cat_val<- FactoMineR::MFA(clustering_new_pre_PCA3_val_vat_agg_scale, 
               group = c(2, 3, 2), 
               type = c("n", "s", "s"),
              # ind.sup = c(1),
              ncp = 2,
...
colnames(resampled_temp_bysubj_cat_val)
resamp_1st_bysubj_cat_val<- data.frame(matrix(NA, nrow = 1, ncol = 12))

# cnames_resamp_subj<- names(resampled_temp_bysubj_cat_val)
colnames(resamp_1st_bysubj_cat_val)<- cnames_resamp_subj
colnames(resamp_1st_bysubj_cat_val)
# create emoty  storage rame for eigenvakues
eigen_first_bysubj_cat_val<- as.data.frame(eugentest)
eigen_first_bysubj_cat_val[,]<- NA

# eigen_first<- as.data.frame(eigen_first)

# create empty storage for correlations
cor_first_bysubj_cat_val<- as.data.frame(cortest)
cor_first_bysubj_cat_val[,]<- NA
# cor_first<- as.data.frame(cor_first)
...
# res.mfa_bio_vs_self2_k2$group$correlation
partials_first_bysubj_cat_val <- as.data.frame(res.mfa_bio_vs_self2_k2[["partial.axes"]][["contrib"]])
partials_first_bysubj_cat_val[,]<- NA
partials_first_bysubj_cat_val
partials_first_bysubj_cat_val$sim_no<- NA
...
groups_first_bysubj_cat_val<-as.data.frame(  res.mfa_bio_vs_self2_k2$group$correlation)
# res.mfa_bio_vs_self2_k2$group$correlation

groups_first_bysubj_cat_val[,]<- NA
groups_first_bysubj_cat_val
groups_first_bysubj_cat_val$sim_no<- NA
...
clustering_new_pre_PCA3_val_vat_agg_scale$valence_iaps_cat<- as.character(clustering_new_pre_PCA3_val_vat_agg_scale$valence_iaps_cat)


clustering_new_pre_PCA3_val_vat_agg_scale$ssid<- as.character(clustering_new_pre_PCA3_val_vat_agg_scale$ssid)
nrow(clustering_new_pre_PCA3_val_vat_agg_scale)

for (i in 1:1000) {
  message(sprintf("$$$$$RUNING sim %i", i))
  
  resampled_temp_bysubj_cat_val<- fabricatr::resample_data(clustering_new_pre_PCA3_val_vat_agg_scale, 
                                  N = c(51,3),
                                  ID_labels = c("ssid", "valence_iaps_cat"),
                                  unique_labels = TRUE)
  # View(resampled_temp_bysubj_cat_val)
  resampled_temp_bysubj_cat_val$sim_no = i
  resampled_temp_bysubj_cat_val$ssid_new <- as.factor(substr(resampled_temp_bysubj_cat_val$valence_iaps_cat_unique,1,6))
  resampled_temp_bysubj_cat_val$newstim <- substr(resampled_temp_bysubj_cat_val$valence_iaps_cat_unique,7,9)

# run PCA on the resampled data
  # colnames(resampled_temp_bysubj_cat_val)
    message(sprintf("$$$$$fitting PCA %i", i))
    # colnames(resampled_temp_bysubj_cat_val[,c(11,12,3:7)])
    mfa.res_bysubj_cat_val <- FactoMineR::MFA(resampled_temp_bysubj_cat_val[,c(11,12,3:7)], 
               group = c(2, 3, 2), 
               type = c("n", "s", "s"),
              # ind.sup = c(1),
              ncp = 2,
...
# store the eigen values
message(sprintf("$$$$$storing PCA %i", i))
eugentest_bysubj_cat_val<- as.data.frame(mfa.res_bysubj_cat_val$eig[1:2,])
eugentest_bysubj_cat_val$sim_no<- i

cortest_bysubj_cat_val<- as.data.frame(mfa.res_bysubj_cat_val$quanti.var$cor)
cortest_bysubj_cat_val$sim_no = i

partials_temp_bysubj_cat_val<- as.data.frame(mfa.res_bysubj_cat_val$partial.axes$cor)
partials_temp_bysubj_cat_val$sim_no = i

# groups
group_temp_bysubj_cat_val<- as.data.frame(mfa.res_bysubj_cat_val$group$cos2)
group_temp_bysubj_cat_val$sim_no = i


# update the initial d=frames
  message(sprintf("$$$$$upating dataframes %i", i))
  resamp_1st_bysubj_cat_val<- (bind_rows(resamp_1st_bysubj_cat_val, resampled_temp_bysubj_cat_val))
eigen_first_bysubj_cat_val<- bind_rows(eigen_first_bysubj_cat_val, eugentest_bysubj_cat_val)
cor_first_bysubj_cat_val<- bind_rows(cor_first_bysubj_cat_val, cortest_bysubj_cat_val)
partials_first_bysubj_cat_val<- bind_rows(partials_first_bysubj_cat_val, partials_temp_bysubj_cat_val)
groups_first_bysubj_cat_val<- bind_rows(groups_first_bysubj_cat_val, group_temp_bysubj_cat_val)
  message(sprintf("$$$$$completed iteration%i", i))
}
...
Visualization of bootsrap for data agregate within particpant and neg mid anpos

```{r}
eigen_first_bysubj_cat_val%>%
  mutate(component =as.factor(substr(rownames(eigen_first_bysubj_cat_val), 1,6)))%>% #retrieve names for components
  subset(!is.na(eigenvalue) & component == "comp 1" )%>% #select only component 1 and remove nas I introduced in the dataframe
  mutate(perc_5 = quantile(eigenvalue,.025, na.rm = TRUE), 
         perc_95 = quantile(eigenvalue,.975, na.rm = TRUE)) %>% #create 95% to descrive interval around the observed estimate
  # mutate(probabcheck= sum(eigenvalue>=res.mfa_bio_vs_self2_k2$eig[1])/n())
...
variables and dimension corelations
```{r}
cor_first_bysubj_cat_val %>%
  mutate(variable = as.factor(substr(rownames(cor_first_bysubj_cat_val), 1,6)))%>%
    subset(!is.na(Dim.1)) %>%
  # create interval per variable
  group_by(variable)%>%
   mutate(perc_5 = quantile(Dim.1,.025, na.rm = TRUE),
...
clustering_new_pre_PCA3.1.scale_agg_bystim
# FactoMineR::MFA(clustering_new_pre_PCA3_val_vat_agg[
clustering_new_pre_PCA3_val_vat_scale_then_agg1

clustering_new_pre_PCA3_val_vat_agg_scale
...
resample for significance testing


```{r}
colnames(resamp_1st_bysubj_reshuf_cat_val)
 # create emopty dataframe
resamp_1st_bysubj_reshuf_cat_val<- data.frame(matrix(NA, nrow = 1, ncol = 8))
cnames_resamp_bysubj_reshuf_cat_val<- names(resampled_temp_bysubj_reshuf_cat_val)

colnames(resamp_1st_bysubj_reshuf_cat_val)<- cnames_resamp_bysubj_reshuf_cat_val
colnames(resamp_1st_bysubj_reshuf_cat_val)
# create emoty  storage rame for eigenvakues
eigen_first_bysubj_reshuf_cat_val<- as.data.frame(eugentest)
eigen_first_bysubj_reshuf_cat_val[,]<- NA

# eigen_first_bysubj_reshuf_cat_val<- as.data.frame(eigen_first_bysubj_reshuf_cat_val)

# create empty storage for correlations
cor_first_bysubj_reshuf_cat_val<- as.data.frame(cortest)
cor_first_bysubj_reshuf_cat_val[,]<- NA
# cor_first_bysubj_reshuf_cat_val<- as.data.frame(cor_first_bysubj_reshuf_cat_val)


# now store group results/partials

# res.mfa_bio_vs_self2_k2$group$correlation
partials_first_bysubj_reshuf_cat_val <- as.data.frame(res.mfa_bio_vs_self2_k2[["partial.axes"]][["contrib"]])
partials_first_bysubj_reshuf_cat_val[,]<- NA
partials_first_bysubj_reshuf_cat_val
partials_first_bysubj_reshuf_cat_val$sim_no<- NA
...
groups_first_bysubj_reshuf_cat_val<-as.data.frame(  res.mfa_bio_vs_self2_k2$group$correlation)
# res.mfa_bio_vs_self2_k2$group$correlation

groups_first_bysubj_reshuf_cat_val[,]<- NA
groups_first_bysubj_reshuf_cat_val
groups_first_bysubj_reshuf_cat_val$sim_no<- NA


#################be careful to use the correct mfa.res_res. a few times i forgot to use the right one and got constants in the updated dataframes (e.g. partials)
?sample

# permutations should be withouth replacement
for (i in 1:1000) {
  message(sprintf("$$$$$RUNING sim %i", i))
  # changed below to replace treue - shuflle with replacement
  # https://stats.stackexchange.com/questions/104040/resampling-simulation-methods-monte-carlo-bootstrapping-jackknifing-cross?rq=1
  # See the question on permutation vs bootstrapping - "The permutation test is best for testing hypotheses and bootstrapping is best for estimating confidence intervals".
  colnames(clustering_new_pre_PCA3_val_vat_agg_scale)
resampled_temp_bysubj_reshuf_cat_val <- clustering_new_pre_PCA3_val_vat_agg_scale %>%
  group_by(ssid)%>%
  transform(pup_basCor = 
              sample(pup_basCor, replace = FALSE),
            Bio_Mean_HR_dif = sample(Bio_Mean_HR_dif, replace = FALSE),
            log_BIO_CDA.PhasicMax = sample(log_BIO_CDA.PhasicMax, replace = FALSE),
            arousal = sample(arousal, replace = FALSE),
            valence = sample(valence, replace = FALSE))
  
  resampled_temp_bysubj_reshuf_cat_val$sim_no = i

# run PCA on the resampled data
# ?list
  # colnames(resampled_temp_bysubj_reshuf_cat_val)
    message(sprintf("$$$$$fitting PCA %i", i))
    mfa.res_bysubj_reshuf_cat_val <- FactoMineR::MFA(resampled_temp_bysubj_reshuf_cat_val[,c(1:7)], 
               group = c(2, 3, 2), 
               type = c("n", "s", "s"),
              # ind.sup = c(1),
              ncp = 2,
...
# store the eigen values
message(sprintf("$$$$$storing PCA %i", i))
eugentest_bysubj_reshuf_cat_vall<- as.data.frame(mfa.res_bysubj_reshuf_cat_val$eig[1:2,])
eugentest_bysubj_reshuf_cat_vall$sim_no<- i

cortest_bysubj_reshuf_cat_vall<- as.data.frame(mfa.res_bysubj_reshuf_cat_val$quanti.var$cor)
cortest_bysubj_reshuf_cat_vall$sim_no = i

partials_temp_bysubj_reshuf_cat_val<- as.data.frame(mfa.res_bysubj_reshuf_cat_val$partial.axes$cor)
partials_temp_bysubj_reshuf_cat_val$sim_no = i

# groups
group_temp_bysubj_reshuf_cat_val<- as.data.frame(mfa.res_bysubj_reshuf_cat_val$group$cos2)
group_temp_bysubj_reshuf_cat_val$sim_no = i


# update the initial d=frames
  message(sprintf("$$$$$upating dataframes %i", i))
  resamp_1st_bysubj_reshuf_cat_val<- (bind_rows(resamp_1st_bysubj_reshuf_cat_val, resampled_temp_bysubj_reshuf_cat_val))
eigen_first_bysubj_reshuf_cat_val<- bind_rows(eigen_first_bysubj_reshuf_cat_val, eugentest_bysubj_reshuf_cat_vall)
cor_first_bysubj_reshuf_cat_val<- bind_rows(cor_first_bysubj_reshuf_cat_val, cortest_bysubj_reshuf_cat_vall)
partials_first_bysubj_reshuf_cat_val<- bind_rows(partials_first_bysubj_reshuf_cat_val, partials_temp_bysubj_reshuf_cat_val)
groups_first_bysubj_reshuf_cat_val<- bind_rows(groups_first_bysubj_reshuf_cat_val, group_temp_bysubj_reshuf_cat_val)
  message(sprintf("$$$$$completed iteration%i", i))
}
...
<!-- visualisae permutation tests -->

```{r}
eigen_first_bysubj_reshuf_cat_val%>%
  mutate(component =as.factor(substr(rownames(eigen_first_bysubj_reshuf_cat_val), 1,6)))%>% #retrieve names for components
  subset(!is.na(eigenvalue) & component == "comp 1" )%>% #select only component 1 and remove nas I introduced in the
  ggplot(aes(eigenvalue))+
  geom_histogram(bins = 500, alpha = .2)+
    geom_density(alpha = .3)+
  # facet_grid(~component)+
    # geom_errorbar(aes(xmin = min(perc_5), xmax=max(perc_95), y = 10), color = "gray30")+
  geom_point(x = res.mfa_agg_bysubj_cat_val$eig[1],y = 10,size = 2, color = "red")+
...
eigen_first_bysubj_reshuf_cat_val%>%
  ...
  # facet_grid(~component)+
    # geom_errorbar(aes(xmin = min(perc_5), xmax=max(perc_95), y = 10), color = "gray30")+
  geom_point(x = res.mfa_agg_bysubj_cat_val$eig[2],y = 10,size = 2, color = "red")+
...
eigen_first_bysubj_reshuf_cat_val%>%
  mutate(component =as.factor(substr(rownames(eigen_first), 1,6)))%>% #retrieve names for components
  subset(!is.na(eigenvalue) & component == "comp 1" )%>%
  mutate(extreme_eigen_count = eigenvalue>= res.mfa_agg_bysubj_cat_val$eig[1])%>%
  mutate(extreme_eigen_count_prob = (sum(extreme_eigen_count))/n())

eigen_first_bysubj_reshuf_cat_val%>%
  mutate(component =as.factor(substr(rownames(eigen_first), 1,6)))%>% #retrieve names for components
  subset(!is.na(eigenvalue) & component == "comp 2" )%>%
  mutate(extreme_eigen_count = eigenvalue>= res.mfa_agg_bysubj_cat_val$eig[2])%>%
  mutate(extreme_eigen_count_prob = (sum(extreme_eigen_count))/n())
...
correlation btween variables and dimensions

```{r}
res.mfa_agg_bysubj$quanti.var$cor

cor_first_bysubj_reshuf_cat_val %>%
  mutate(variable = as.factor(substr(rownames(cor_first_bysubj_reshuf_cat_val), 1,6)))%>%
    subset(!is.na(Dim.1)) %>%
  group_by(variable)%>%
  ggplot(aes(Dim.1)) +
  geom_histogram(aes(y=..density..), alpha=0.5,
...
cor_first_bysubj_reshuf_cat_val %>%
  ...
  group_by(variable)%>%
   ...
   #       perc_95 = quantile(Dim.1,.975, na.rm = TRUE))%>%
  subset(variable == "Bio_Me")%>%
  mutate(hr_cor_extreme_d1 = abs(Dim.1)>= abs(mfa.res_bysubj_reshuf_cat_val$quanti.var$cor[2,1]),
         hr_cor_extreme_d2 = abs(Dim.2)>= abs(mfa.res_bysubj_reshuf_cat_val$quanti.var$cor[2,2]))%>%
  mutate(prob_extree_hr_d1 = (sum(hr_cor_extreme_d1))/n(),
         prob_extree_hr_d2 = (sum(hr_cor_extreme_d2))/n())
...
```{r}
res.mfa_agg_bysubj_cat_val$group$correlation
groups_first_bysubj_cat_val%>%
  mutate(group = as.factor(substr(rownames(groups_first_bysubj_cat_val), 1,4)))%>%
    subset(!is.na(Dim.1)) %>%
  # create interval per variable
...
```{r}
colnames(clustering_new_pre_PCA3.1.scale_agg_bystim_scaleafteragg)

res.mfa_agg_stim<- FactoMineR::MFA(clustering_new_pre_PCA3.1.scale_agg_bystim_scaleafteragg, 
               group = c(2, 3, 2), 
               type = c("n", "s", "s"),
               # "s", "s", "s"
               name.group = c("stim_cat","auton","subj"),
               num.group.sup = c(1),
               # ind.sup = c(1,2),
               graph = TRUE)
...
clustering_new_pre_PCA3.1.scale_agg_bystim_scaleafteragg # this is the one I need
colnames(clustering_new_pre_PCA3.1.scale_agg_bystim_scaleafteragg)


colnames(resampled_temp_bystim_cat_val)
resamp_1st_bystim_cat_val<- data.frame(matrix(NA, nrow = 1, ncol = 10))

cnames_resamp_subj<- names(resampled_temp_bystim_cat_val)
colnames(resamp_1st_bystim_cat_val)<- cnames_resamp_subj
colnames(resamp_1st_bystim_cat_val)
# create emoty  storage rame for eigenvakues
eigen_first_bystim_cat_val<- as.data.frame(eugentest)
eigen_first_bystim_cat_val[,]<- NA

# eigen_first<- as.data.frame(eigen_first)

# create empty storage for correlations
cor_first_bystim_cat_val<- as.data.frame(cortest)
cor_first_bystim_cat_val[,]<- NA
# cor_first<- as.data.frame(cor_first)
...
# res.mfa_bio_vs_self2_k2$group$correlation
partials_first_bystim_cat_val <- as.data.frame(res.mfa_bio_vs_self2_k2[["partial.axes"]][["contrib"]])
partials_first_bystim_cat_val[,]<- NA
partials_first_bystim_cat_val
partials_first_bystim_cat_val$sim_no<- NA


# hierarquical clustering
clustering_new_pre_PCA3.1.scale_agg_bystim_scaleafteragg$stimDescription <- as.character(clustering_new_pre_PCA3.1.scale_agg_bystim_scaleafteragg$stimDescription)
clustering_new_pre_PCA3.1.scale_agg_bystim_scaleafteragg$val_cut_iaps <- as.character(clustering_new_pre_PCA3.1.scale_agg_bystim_scaleafteragg$val_cut_iaps)


# think we need to store groups

groups_first_bystim_cat_val<-as.data.frame(  res.mfa_bio_vs_self2_k2$group$correlation)
# res.mfa_bio_vs_self2_k2$group$correlation

groups_first_bystim_cat_val[,]<- NA
groups_first_bystim_cat_val
groups_first_bystim_cat_val$sim_no<- NA
...
for (i in 1:1000) {
  ...
  resampled_temp_bystim_cat_val<- fabricatr::resample_data(clustering_new_pre_PCA3.1.scale_agg_bystim_scaleafteragg, 
                                  N = c(53),
                                  ID_labels = c("stimDescription"),
                                  unique_labels = TRUE)
  # View(resampled_temp_bystim_cat_val)
  resampled_temp_bystim_cat_val$sim_no = i
  resampled_temp_bystim_cat_val$stimDescription_new <- as.factor(substr(resampled_temp_bystim_cat_val$stimDescription_unique,1,3))

# run PCA on the resampled data
  # colnames(resampled_temp_bystim_cat_val)
    message(sprintf("$$$$$fitting PCA %i", i))
    # colnames(resampled_temp_bystim_cat_val[,c(10,2:7)])
    mfa.res_bystim_cat_val <- FactoMineR::MFA(resampled_temp_bystim_cat_val[,c(10,2:7)], 
               group = c(2, 3, 2), 
               type = c("n", "s", "s"),
              # ind.sup = c(1),
              ncp = 2,
               name.group = c("stimid_cat","auton","subj"),
               num.group.sup = c(1),
               # ind.sup = c(1,2),
               graph = FALSE)
...
# store the eigen values
message(sprintf("$$$$$storing PCA %i", i))
eugentest_bystim_cat_val<- as.data.frame(mfa.res_bystim_cat_val$eig[1:2,])
eugentest_bystim_cat_val$sim_no<- i

cortest_bystim_cat_val<- as.data.frame(mfa.res_bystim_cat_val$quanti.var$cor)
cortest_bystim_cat_val$sim_no = i

partials_temp_bystim_cat_val<- as.data.frame(mfa.res_bystim_cat_val$partial.axes$cor)
partials_temp_bystim_cat_val$sim_no = i

# groups
group_temp_bystim_cat_val<- as.data.frame(mfa.res_bystim_cat_val$group$cos2)
group_temp_bystim_cat_val$sim_no = i


# update the initial d=frames
  message(sprintf("$$$$$upating dataframes %i", i))
  resamp_1st_bystim_cat_val<- (bind_rows(resamp_1st_bystim_cat_val, resampled_temp_bystim_cat_val))
eigen_first_bystim_cat_val<- bind_rows(eigen_first_bystim_cat_val, eugentest_bystim_cat_val)
cor_first_bystim_cat_val<- bind_rows(cor_first_bystim_cat_val, cortest_bystim_cat_val)
partials_first_bystim_cat_val<- bind_rows(partials_first_bystim_cat_val, partials_temp_bystim_cat_val)
groups_first_bystim_cat_val<- bind_rows(groups_first_bystim_cat_val, group_temp_bystim_cat_val)
  message(sprintf("$$$$$completed iteration%i", i))
}

#################################
# note i have not checked the rsults from above
...
```{r}
eigen_first_bystim_cat_val%>%
  mutate(component =as.factor(substr(rownames(eigen_first_bystim_cat_val), 1,6)))%>% #retrieve names for components
  subset(!is.na(eigenvalue) & component == "comp 1" )%>% #select only component 1 and remove nas I introduced in the dataframe
  mutate(perc_5 = quantile(eigenvalue,.025, na.rm = TRUE), 
         perc_95 = quantile(eigenvalue,.975, na.rm = TRUE)) %>% #create 95% to descrive interval around the observed estimate
  # mutate(probabcheck= sum(eigenvalue>=res.mfa_bio_vs_self2_k2$eig[1])/n())
  ...
  # facet_grid(~component)+
    geom_errorbar(aes(xmin = min(perc_5), xmax=max(perc_95), y = 10), color = "gray30")+
  geom_point(x = res.mfa_agg_stim$eig[1],y = 10,size = 2, color = "red")+
...
eigen_first_bystim_cat_val%>%
  ...
  # facet_grid(~component)+
    geom_errorbar(aes(xmin = min(perc_5), xmax=max(perc_95), y = 10), color = "gray30")+
  geom_point(x = res.mfa_agg_stim$eig[2],y = 10,size = 2, color = "red")+
...
```{r}
cor_first_bystim_cat_val %>%
  mutate(variable = as.factor(substr(rownames(cor_first_bystim_cat_val), 1,6)))%>%
    subset(!is.na(Dim.1)) %>%
  # create interval per variable
  group_by(variable)%>%
   mutate(perc_5 = quantile(Dim.1,.025, na.rm = TRUE),
...
```{r}
groups_first_bystim_cat_val%>%
  mutate(group = as.factor(substr(rownames(groups_first_bystim_cat_val), 1,4)))%>%
    subset(!is.na(Dim.1)) %>%
  # create interval per variable
...
resdshufle stim
```{r}
colnames(resampled_temp_bystimj_reshuf)
# create emopty dataframe
resamp_1st_bystimj_reshuf<- data.frame(matrix(NA, nrow = 1, ncol = 8))
cnames_resamp_bystimj_reshuf<- names(resampled_temp_bystimj_reshuf)

colnames(resamp_1st_bystimj_reshuf)<- cnames_resamp_bystimj_reshuf
colnames(resamp_1st_bystimj_reshuf)
# create emoty  storage rame for eigenvakues
eigen_first_bystimj_reshuf<- as.data.frame(eugentest)
eigen_first_bystimj_reshuf[,]<- NA

# eigen_first_bystimj_reshuf<- as.data.frame(eigen_first_bystimj_reshuf)

# create empty storage for correlations
cor_first_bystimj_reshuf<- as.data.frame(cortest)
cor_first_bystimj_reshuf[,]<- NA
# cor_first_bystimj_reshuf<- as.data.frame(cor_first_bystimj_reshuf)


# now store group results/partials

# res.mfa_bio_vs_self2_k2$group$correlation
partials_first_bystimj_reshuf <- as.data.frame(res.mfa_bio_vs_self2_k2[["partial.axes"]][["contrib"]])
partials_first_bystimj_reshuf[,]<- NA
partials_first_bystimj_reshuf
partials_first_bystimj_reshuf$sim_no<- NA
...
groups_first_bystimj_reshuf<-as.data.frame(  res.mfa_bio_vs_self2_k2$group$correlation)
# res.mfa_bio_vs_self2_k2$group$correlation

groups_first_bystimj_reshuf[,]<- NA
groups_first_bystimj_reshuf
groups_first_bystimj_reshuf$sim_no<- NA
...
for (i in 1:1000) {
  ...
  # https://stats.stackexchange.com/questions/104040/resampling-simulation-methods-monte-carlo-bootstrapping-jackknifing-cross?rq=1
  # See the question on permutation vs bootstrapping - "The permutation test is best for testing hypotheses and bootstrapping is best for estimating confidence intervals".
resampled_temp_bystimj_reshuf <- clustering_new_pre_PCA3.1.scale_agg_bystim_scaleafteragg %>%
  # group_by(ssid)%>%
  transform(pup_basCor = 
            sample(pup_basCor, replace = FALSE),
            Bio_Mean_HR_dif = sample(Bio_Mean_HR_dif, replace = FALSE),
...
# View(resampled_temp_bystimj_reshuf)
  resampled_temp_bystimj_reshuf$sim_no = i

# run PCA on the resampled data
# ?list
  # colnames(resampled_temp_bystimj_reshuf)
    message(sprintf("$$$$$fitting PCA %i", i))
    mfa.res_bystimj_reshuf <- FactoMineR::MFA(resampled_temp_bystimj_reshuf[,c(1:7)], 
               group = c(2, 3, 2), 
               type = c("n", "s", "s"),
              # ind.sup = c(1),
              ncp = 2,
...
# store the eigen values
message(sprintf("$$$$$storing PCA %i", i))
eugentest_bystimj_reshufl<- as.data.frame(mfa.res_bystimj_reshuf$eig[1:2,])
eugentest_bystimj_reshufl$sim_no<- i

cortest_bystimj_reshufl<- as.data.frame(mfa.res_bystimj_reshuf$quanti.var$cor)
cortest_bystimj_reshufl$sim_no = i

partials_temp_bystimj_reshuf<- as.data.frame(mfa.res_bystimj_reshuf$partial.axes$cor)
partials_temp_bystimj_reshuf$sim_no = i

# groups
group_temp_bystimj_reshuf<- as.data.frame(mfa.res_bystimj_reshuf$group$cos2)
group_temp_bystimj_reshuf$sim_no = i


# update the initial d=frames
  message(sprintf("$$$$$upating dataframes %i", i))
  resamp_1st_bystimj_reshuf<- (bind_rows(resamp_1st_bystimj_reshuf, resampled_temp_bystimj_reshuf))
eigen_first_bystimj_reshuf<- bind_rows(eigen_first_bystimj_reshuf, eugentest_bystimj_reshufl)
cor_first_bystimj_reshuf<- bind_rows(cor_first_bystimj_reshuf, cortest_bystimj_reshufl)
partials_first_bystimj_reshuf<- bind_rows(partials_first_bystimj_reshuf, partials_temp_bystimj_reshuf)
groups_first_bystimj_reshuf<- bind_rows(groups_first_bystimj_reshuf, group_temp_bystimj_reshuf)
  message(sprintf("$$$$$completed iteration%i", i))
}
...
visualise pernuation on stim MFA

```{r}
eigen_first_bystimj_reshuf %>%
  mutate(component =as.factor(substr(rownames(eigen_first_bystimj_reshuf), 1,6)))%>% #retrieve names for components
  subset(!is.na(eigenvalue) & component == "comp 1" )%>% #select only component 1 and remove nas I introduced in the
  ggplot(aes(eigenvalue))+
  geom_histogram(bins = 500, alpha = .2)+
    geom_density(alpha = .3)+
  ...
  theme_classic()+
  geom_vline(xintercept = res.mfa_agg_stim$eig[1], linetype = "dashed")
...
eigen_first_bystimj_reshuf %>%
  ...
  theme_classic()+
  geom_vline(xintercept = res.mfa_agg_stim$eig[2], linetype = "dashed")
# not significant
...
eigen_first_bystimj_reshuf%>%
  mutate(component =as.factor(substr(rownames(eigen_first), 1,6)))%>% #retrieve names for components
  subset(!is.na(eigenvalue) & component == "comp 1" )%>%
  mutate(extreme_eigen_count = eigenvalue>= res.mfa_agg_stim$eig[1])%>%
  mutate(extreme_eigen_count_prob = (sum(extreme_eigen_count))/n())

eigen_first_bystimj_reshuf%>%
  mutate(component =as.factor(substr(rownames(eigen_first), 1,6)))%>% #retrieve names for components
  subset(!is.na(eigenvalue) & component == "comp 2" )%>%
  mutate(extreme_eigen_count = eigenvalue>= res.mfa_agg_stim$eig[2])%>%
  mutate(extreme_eigen_count_prob = (sum(extreme_eigen_count))/n())

```


significance variable and dimensions



```{r}

res.mfa_agg_stim$quanti.var$cor
cor_first_bystim_cat_val %>%
  mutate(variable = as.factor(substr(rownames(cor_first_bystim_cat_val), 1,6)))%>%
    subset(!is.na(Dim.1)) %>%
  group_by(variable)%>%
...
?cut
#create valence categories based ion self reports (IAPS validation)
clustering_new_pre_PCA3_val_vat<- clustering_new_pre_PCA3%>%
  mutate(valence_iaps_cat = cut(ValenceMean,3, labels = c("neg", "mid", "pos")))
  

# scale within subject
# aggregate by subject and 

View(clustering_new_pre_PCA3_val_vat)
# clustering_new_pre_PCA3_val_vat%>%
#   group_by(ssid)%>%
#   mutate_if(is.numeric, scale)
  
clustering_new_pre_PCA3_val_vat_agg<- clustering_new_pre_PCA3_val_vat%>%
  group_by(ssid, valence_iaps_cat)%>%
  summarise_if(is.numeric, mean, na.rm = TRUE)

library(factoextra)
library(FactoMineR) 

colnames(clustering_new_pre_PCA3_val_vat_agg)
clustering_new_pre_PCA3_val_vat_agg[,c(2,5,6,7,8,9)]


clustering_new_pre_PCA3.1.scale_agg_bystim
FactoMineR::MFA(clustering_new_pre_PCA3_val_vat_agg[,c(1,2,5,6,7,8,9)], 
               group = c(2, 3, 2), 
               type = c("n", "s", "s"),
               # "s", "s", "s"
               name.group = c("ss_id_stim","auton","subj"),
               num.group.sup = c(1),
               # ind.sup = c(1,2),
               graph = TRUE)
...
# scale
clustering_new_pre_PCA3_val_vat_agg_scale<- clustering_new_pre_PCA3_val_vat_agg[,c(1,2,5,6,7,8,9)]
...
clustering_new_pre_PCA3_val_vat_scale_then_agg<- clustering_new_pre_PCA3_val_vat%>%
  group_by(ssid)%>%
  mutate_if(is.numeric, scale)%>%
  group_by(ssid, valence_iaps_cat)%>%
  summarise_if(is.numeric, mean, na.rm = TRUE)
...
clustering_new_pre_PCA3_val_vat_scale_then_agg1<- clustering_new_pre_PCA3_val_vat_scale_then_agg[,c(1:2, 5:9)]

?FactoMineR::MFA
res.mfa_val_cat_scal<- FactoMineR::MFA(clustering_new_pre_PCA3_val_vat_scale_then_agg1,
               group = c(2, 3, 2), 
               type = c("n", "s", "s"),
               name.group = c("ss_id_stim","auton","subj"),
               num.group.sup = c(1),
...
?FactoMineR::MFA
View(res.mfa_val_cat_scal)

# force k = 2
# res.mfa_bio_vs_self2_k2

res.mfa_val_cat_scal_k2<- FactoMineR::MFA(clustering_new_pre_PCA3_val_vat_scale_then_agg1,
               group = c(2, 3, 2), 
               type = c("n", "s", "s"),
               name.group = c("ss_id_stim","auton","subj"),
               num.group.sup = c(1),
...
fviz_mfa_ind(res.mfa_val_cat_scal, 
             habillage = "valence_iaps_cat", # color by groups 
             # palette = c("#00AFBB", "#E7B800", "#FC4E07"),
             addEllipses = TRUE, 
             ellipse.type = "confidence", 
             repel = TRUE # Avoid text overlapping
...
# we can sorta see that while things allign relativelly well on sujective dimension, they don't allugn very well on the autonomic dimension
