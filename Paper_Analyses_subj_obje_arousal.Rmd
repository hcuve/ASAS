...
tmp.df4_full_stim_downs_jun2021_60bins%>%
  subset(!is.na(ground_valence))%>%
  group_by(ssid,ground_valence)%>%
  mutate(mean_abs_arousal = mean(abs(arousal), na.rm = TRUE))%>%
  summarise_at(mean_abs_arousal)


tmp.df4_full_stim_downs_jun2021_60bins%>%
  group_by(stimIAPS, ssid,ground_valence_2)%>%
  summarise_if(is.numeric, mean, na.rm = TRUE)%>%
  ggplot(aes(stimIAPS, abs(arousal), color = ground_valence_2))+
  # geom_point()+
  stat_summary(funy = mean, geom = 'pointrange')
