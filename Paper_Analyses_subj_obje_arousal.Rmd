...
timecourse_60bin_result_df %>%
  ...
  geom_line(size = 1, linetype = 'dotted', color = 1)+
  # geom_line(aes(y = Estimate_vl), linetype = "dashed", size = 1)+
  geom_line(aes(y = Estimate_br), size = 1, color = 2)+
  # geom_line(aes(y = Estimate_ar_vl), linetype = "dotdash", size = 1, color = "red")+
  geom_rect(aes(xmin = 1,
                xmax = 5.89,
                ymin = -2,
                ymax = -2+.1,
                fill = "red", linetype = 'dashed'), alpha = .01)+
  p$graphstyle+
  scale_color_brewer(palette="Dark2")
...
library(RColorBrewer)
  ...
  ggplot(aes(timebins, Estimate, group = variable, fill = variable, color= variable))+
  geom_line(aes(linetype = variable), size = 1)+
    p$graphstyle+
    scale_color_manual(values=c("#1B9E77", "#D95F02", "#7570B3", "#E7298A"))+
  # use correction for significance
    # brighteness
   ...
   # timecourse_60bin_result_df$p_ar_vl_adj
    # timecourse_60bin_result_df$timebins[timecourse_60bin_result_df$p_ar_vl_adj_TRUE == TRUE]
       annotate(geom = 'rect', 
                ...
                ymin = -2+.1+.1+.1+.1,
                ymax = -2+.1+.1+.1+.1+.1,
                fill = "#D95F02", alpha = .5, label = "text")+
    annotate("text", 
                x = (1+5.4)/2,
                y = ((-2+.1+.1+.1+.1) +(-2+.1+.1+.1+.1+.1))/2,
...
velocity = actual velocity unlike speed (which is what I actually computed above. velocity encodes durection( constriction or dilation)
