...
pupil_arousal_findings$pupil_from_ar_vale

library(insight)
sjPlot::sjp.lmer(pupil_arousal_findings$pupil_from_ar_vale)
...
min(clustering_new_pre_PCA3.1$log_BIO_CDA.PhasicMax, na.rm = TRUE)
testjustpca <-InPosition::epPCA.inference.battery(clustering_new_pre_PCA3.1[,5:7], 
                                       scale = TRUE, 
                                       center = TRUE, 
                                       # DESIGN = var.design_asd, 
                                       # make_design_nominal = TRUE, 
...
clustering_new_pre_PCA3.1$pc1<- testjustpca$Fixed.Data$ExPosition.Data$fi[,1]
clustering_new_pre_PCA3.1$pc2<- testjustpca$Fixed.Data$ExPosition.Data$fi[,2]
testjustpca$Fixed.Data$ExPosition.Data$fi

library(lmerTest)

summary(lmer(arousal ~ (pc1 + pc2) + (1|ssid) + (1|stimDescription),
     data = clustering_new_pre_PCA3.1))

write_csv(clustering_new_pre_PCA3.1, "clustering_new_pre_PCA3.1new.csv")
...
# 
colnames(clustering_new_pre_PCA_2_agg_by_val_sel)

min(clustering_new_pre_PCA_2_agg_by_val_sel$log_BIO_CDA.PhasicMax)
testjustpca2 <-InPosition::epPCA.inference.battery(clustering_new_pre_PCA_2_agg_by_val_sel[,3:5], 
                                       scale = TRUE, 
                                       center = TRUE, 
                                       # DESIGN = var.design_asd, 
                                       # make_design_nominal = TRUE, 
...
clustering_new_pre_PCA_2_agg_by_val_sel$pc1 <- testjustpca2$Fixed.Data$ExPosition.Data$fi[,1]
clustering_new_pre_PCA_2_agg_by_val_sel$pc2 <- testjustpca2$Fixed.Data$ExPosition.Data$fi[,2]



library(lmerTest)

summary(lmer(arousal ~ (pc1 + pc2) + (1|ssid),
     data = clustering_new_pre_PCA_2_agg_by_val_sel))
...
groups_first%>%
  ...
  res.MFA_stim_val_scale$group$contrib
  res.MFA_stim_val_scale[["ind"]][["coord"]])


```


Do we get the same patterns when looking at participants and stimuli separately
...
# create emoty  storage rame for eigenvakues
eigen_first_reshuf<- as.data.frame(res.mfa_bio_vs_self2_k2$eig)
...
save(clustering_new_pre_PCA3.1_copy,resampled_temp_reshuf,
     resamp_1st_reshuf,
     eigen_first_reshuf,
     cor_first_reshuf,
     res.mfa_bio_vs_self2_k2,
     clustering_new_pre_PCA3.1, file = "resample_hierar.RData")
...
eigen_first_reshuf%>%
  ...
  mutate(eig_obs_dim = if_else(component == "comp 1", 
                               ...
                               res.mfa_bio_vs_self2_k2$eig[2])) %>% #store observed to visualise ion the null
...
cor_first_reshuf1<- cor_first_reshuf %>%
...
left_join(cor_first_reshuf1, cor.test_obsmfa, by = c("variable", "dimension")) %>%
  group_by(variable,dimension)%>%
...
partials_first_reshuf_1<- partials_first_reshuf %>%
  mutate(dimension_partial = as.factor(substr(rownames(partials_first_reshuf), 1,4)),
...
left_join(partials_first_reshuf_1,partials_first_1_obs,by = c("dimension_partial", "group", "dimension")) %>%
  group_by(dimension_partial,dimension, group)%>%
   # group_by(variable,dimension)%>%
     mutate(prob1 = paste(((sum(abs(value_dimenion.x)>=abs(value_dimenion.y)))/n())*100, paste0("%")))%>%
   mutate(prob = if_else(((sum(abs(value_dimenion.x)>=abs(value_dimenion.y))/n())*100)<5, "p < .05", "ns")) %>%
...
group_cordf1<- group_cordf%>%
gather(dimension, value_dimenion, - group)

groups_first_reshuf1<- left_join(groups_first_reshuf, group_cordf1)


 
# View(groups_first_reshuf)
groups_first_reshuf1<- groups_first_reshuf %>%
  gather(dimension, value_dimenion, -group, -sim_no) %>%
  subset(!is.na(value_dimenion))
  
 left_join(groups_first_reshuf1, group_cordf1, c("group", "dimension"))%>%
   group_by(group,dimension)%>%
