...
plots_paper$interaction_ar_vl <- plots_paper$interaction_ar_vl +
 guides(title="valence",
        fill = guide_legend(title="valence"),
        color = guide_legend(title="valence"),
        linetype = guide_legend(title="valence"))
...
plots_paper$pupil_correlation <- db_full4new_stim_screen_pupil_nopract %>%
  ...
  ggpubr::stat_compare_means(method = 't.test',
   ...
   ylab(paste0("Pupil vs subjective \n arousal (cor)"))+
...
plots_timecourse$eff_size_pup_amp+ p$graphstyle
...
plots_paper$pupil_correlation <-plots_paper$pupil_correlation +
  theme(legend.position = "none")
fix_dur_byval<-
fix_dur_byval+theme(legend.position = "none")
part1<- (plots_paper$interaction_ar_vl |plots_paper$pupil_correlation |plots_timecourse$eff_size_pup_amp)/
  (plots_paper$interaction_SCR| plots_paper$bio_HR |scater_pupil_HR)
    
part2<-    (correlation_plot+fix_dur_byval)+plot_layout(ncol = 2, widths = c(3,1))

part3<- (part1/
part2)
  # plot_layout(nrow = 3)+
  plot_annotation(tag_levels = "A", widths = c(2,1,1), heigths = c(1,1,2))
...
ggsave("part3.tiff", part3, device = "tiff", width = 15,height = 12, dpi = 800)
```
...
scater_pupil_HR<- db_full6new_hr_fix_stim_nt1.1 %>%
  ...
  geom_smooth(method = 'lm', se = F)+
  ggpubr::stat_cor()+
  xlab(expression("Mean "*Delta*" heart rate (bpm)"))+
...
db_full4new_stim_screen_pupil_nopract %>%
  ...
  ggplot(aes(mediansplit_ground_valence, mean_fix_dur, color = mediansplit_ground_valence, fill = mediansplit_ground_valence))+
