...
hdf5FilesPath<- "~/OneDrive - Nexus365/InteroStudy2020/analysis/DataAnalysisJanuary2020/DataAnalysisJan2020/Eyetracking/2021/346_357_611_616"

setwd("~/OneDrive - Nexus365/InteroStudy2020/analysis/DataAnalysisJanuary2020/DataAnalysisJan2020/Eyetracking/2021/346_357_611_616")
...
tmp.df2$ssid[tmp.df2$ssid == 311] <- 611
tmp.df2$ssid[tmp.df2$ssid == 312] <- 357
unique(tmp.df2$ssid)
...
tmp.df3$gsr_ms[tmp.df3$gsr_ms == "Inf"]<-NA

# gsr for leda
colnames(tmp.df3)
db_GSR_backup<- db_GSR
db_GSR<- select(tmp.df3, c(3,  14, 17, 50, 49,51,52 ))
db_GSR$tNo<- NULL
colnames(db_GSR_backup)
# unique(db_GSR_backup$trigger)
colnames(db_GSR)
...
range(tmp.df3$gsr_basCor)
tmp.df3 <- tmp.df3 %>%
    group_by(ssid, tNo, gsr_resp_Log )%>%
    mutate(gsr_response = max(gsr_basCor, na.rm = TRUE))

range(tmp.df3$gsr_response, na.rm = TRUE)
unique(tmp.df3$gsr_response, na.rm = TRUE)

tmp.df3$gsr_response[tmp.df3$gsr_response == "-Inf"]<- NA
...
# tmp.df3$pup_basCor<- if_else(tmp.df3$speed< tmp.df3$MAD, tmp.df3$pup_basCor, NULL)
  ...
  subset(tmp.df3,  tNo == 10 & ssid == 357)%>%
ggplot(aes(x = timerezero3, y= pup_basCor))+
    # geom_smooth(se = F)+
  geom_line()+
    geom_line(aes(y =pup_interp_mad), color = "red")+
    geom_vline(aes(xintercept = 0), colour = 'red', linetype = 'dashed', size = 2)+
    geom_vline(aes(xintercept = 4), colour = 'gray30', linetype = 'dotted', size = 2)+
    geom_vline(aes(xintercept = 8), colour = 'gray30', linetype = 'dotted', size = 2)+
    geom_vline(aes(xintercept = 6), colour = 'blue', linetype = 'dashed', size = 2)
...
tmp.df3<- tmp.df3%>%
  ...
  min(tmp.df3$hr, na.rm = TRUE)
ggplot(subset(tmp.df3, tmp.df3$tNo == -7 & tmp.df3$ssid == 611) , aes(x = timerezero3, y= hr_interp))+
    # geom_smooth(se = F)+
  geom_line()+
...
ggplot(subset(tmp.df4, tmp.df3$ssid == 615) , aes(x = tNo, y= hr_valid_prop))+
    # geom_smooth(se = F)+
  geom_line()+
  geom_line(aes(y = hr_loss_prop), linetype = 'dashed', colour = 'green', size = 0.3)
   # geom_line(aes(y = hr), linetype = 'dotted', colour = 'blue', size = 0.4)

ggplot(subset(tmp.df4, tmp.df3$ssid == 356) , aes(x = tNo, y= gaze_valid_prop))+
    # geom_smooth(se = F)+
  geom_line()+
...
write_csv(tmp.df4, 'interodata_clean_gp_356_357_611_616.csv')
...
write_csv(tmp.df4_sum, 'interodata_clean_gp_356_357_611_616.csv')

# interodata_clean_gp_356_357_611_616.csv
# 
# head(tmp.df4_sum)
...
# 616 seesm to have vey poor data fr eyetracking - exclude
...
write_csv(db_parsed, 'db_parsed_356_357_611_615.csv')
...
write_csv(db_parsed_summarised2, 'db_parsed_summarised_356_357_611_615.csv')
...
setwd("~/OneDrive - Nexus365/InteroStudy2020/analysis/DataAnalysisJanuary2020/DataAnalysisJan2020/IAPSBehav/356_357_611_616")
...
tmp.df4_sum_full_backup_before_beh<- tmp.df4_sum_full
...
library(readxl)
db_selfrep_backup<- db_selfrep
db_selfrep <- read_excel("db_self_rep_345_356_final.xlsx")

library(readr)

db_selfrep <- read_csv("db_self_rep_356_357_611_616_final.csv")
View(db_selfrep)
...
unique(db_selfrep$ssid )

tmp.df4_sum_full_backup_before_selfrep<- tmp.df4_sum_full
db_selfrep$`Participant Private ID.x.x`<- NULL
...
write_csv(tmp.df4_sum_full, 'tmp.df4_sum_full_356_357_611_616.csv')
# tmp.df4_sum_full<- tmp.df4_sum_full%>%
#   arrange(ssid, tNo, screencontent_no)
unique(tmp.df4_sum_full$ssid)
colnames(tmp.df4_sum_full)
...
>> Ledalab('/Users/heliocuve/OneDrive - Nexus365/InteroStudy2020/analysis/DataAnalysisJanuary2020/DataAnalysisJan2020/PhysioLedalab/GP_gsr_forLeda/346-355_2021/3052_356/', 'open','text3','downsample', 15, 'smooth',{'adapt'}, 'analyze','CDA', 'optimize',3, 'export_era', [1 6 .01 2])

>> Ledalab('/Users/heliocuve/OneDrive - Nexus365/InteroStudy2020/analysis/DataAnalysisJanuary2020/DataAnalysisJan2020/PhysioLedalab/GP_gsr_forLeda/356_457_611_616/', 'open','text3','downsample', 15, 'smooth',{'adapt'}, 'analyze','CDA', 'optimize',3, 'export_era', [1 6 .01 2])


>> Ledalab('/Users/heliocuve/OneDrive - Nexus365/InteroStudy2020/analysis/DataAnalysisJanuary2020/DataAnalysisJan2020/PhysioLedalab/ledalab-349/', 'open','biopac','downsample', 200, 'smooth',{'adapt'}, 'analyze','CDA', 'optimize',3, 'export_era', [1 6 .01 2]
...
setwd("~/OneDrive - Nexus365/InteroStudy2020/analysis/DataAnalysisJanuary2020/DataAnalysisJan2020/PhysioLedalab/GP_gsr_forLeda/356_457_611_616/Processed")
...
tmp.df4_sum2_backup<- tmp.df4_sum2
...
# check stiom screen
tmp.df4_sum_full_stim_baclup<- tmp.df4_sum_full_stim
...
```{r}
setwd("~/OneDrive - Nexus365/InteroStudy2020/analysis/DataAnalysisJanuary2020/DataAnalysisJan2020/PhysioLedalab/IAPS/processed/356_357_611_616/CDABIO")
...
setwd("~/OneDrive - Nexus365/InteroStudy2020/Data Collection 2019- 2020/2020/InteroStudyBIOPAC/IAPS/Processed/HR/356_357_611_616")
library(readxl)
library(dplyr)
...
# now read them
db_HR_bionew_backup<- db_HR_bionew
...
# db_HR_bionew<- subset(db_HR_bionew,db_HR_bionew$`Stim-Response Analysis`!= "Stimulus 100")
...
# subset(db_HR_bio, db_HR_bio$ssid== 357)%>%
#  group_by(timestart, ssid)%>%
# mutate(EventNR= 1:224)
...
db_HR_bio2.1_backup<- db_HR_bio2.1
...
unique(db_HR_bio2.1_backup$tNo)

table(db_HR_bio2.1$ssid)
# 
# test<- subset(db_HR_bio2.1, ssid == "357")
# 
# test$timestart
# sort(as.numeric(substr(test$stim, 10,12)))
# 
# arrange(as.numeric(unique(test$stim)))
# 
...
table(db_HR_bio2.1_backup$tNo3)

test352<- subset(db_HR_bio2.1, ssid == 356)
...
write_csv(db_HR_bio_final, 'db_HR_bio_final_356_357_611_616_withfix.csv')
...
tmp.df4_sum_full_bio1_backup<- tmp.df4_sum_full_bio1
...
tmp.df4_sum_full_bio_hr_backlup<- tmp.df4_sum_full_bio_hr
...
unique(tmp.df4_sum_full_bio_hr$ssid)
...
tmp.df4_sum_full_bio_hr_backlup
tmp.df4_sum_full_bio_hr_self_backup<- tmp.df4_sum_full_bio_hr_self
...
write_csv(tmp.df4_sum_full_bio_hr_self, 'tmp.df4_sum_full_bio_hr_self_356_357_611_616.csv')
...
unique(db_parsed_summarised2$ssid)


db_parsed_summarised2$ssid<- as.character(db_parsed_summarised2$ssid)
tmp.df4_sum_full_bio_hr_self_parsed_backup<- tmp.df4_sum_full_bio_hr_self_parsed

tmp.df4_sum_full_bio_hr_self_parsed <- left_join(tmp.df4_sum_full_bio_hr_self, 
                                                 db_parsed_summarised2, 
...
tmp.df4_sum_full_bio_hr_self_parsed_beforebehav_backup<- tmp.df4_sum_full_bio_hr_self_parsed
...
write_csv(tmp.df4_sum_full_bio_hr_self_parsed, 'tmp.df4_sum_full_bio_hr_self_parsed_356_357_611_616.csv')
write_csv(tmp.df4, 'tmp.df4_timeseries_356_357_611_616.csv')

save.image("~/OneDrive - Nexus365/InteroStudy2020/analysis/DataAnalysisJanuary2020/DataAnalysisJan2020/356_357_611_616_preprocessing.RData")
```
