...
# for some reason 355 is creating duplicate event ids
library(readr)
ss355_GPdata_ <- read_csv("ss355_GPdata .csv")

View(ss355_GPdata_)

ss355_GPdata_nodup <- subset(ss355_GPdata_, !duplicated(ss355_GPdata_$event_id))

write_csv(ss355_GPdata_nodup, "ss355_GPdata_nodup.csv")
...
# now read them
# setwd("D:/OneDrive - Nexus365/InteroStudy2020/analysis/DataAnalysisJanuary2020/DataAnalysisJan2020/Step1_GpExtractor_Output")
setwd("~/OneDrive - Nexus365/InteroStudy2020/analysis/DataAnalysisJanuary2020/DataAnalysisJan2020/Eyetracking/2021/step1")
...
gp_preprocessing = function(tmp.df2) {
  ...
  message(sprintf("step 4 cuting off screen in points and repixel"))
...
# let's check to see if it looks fine - NOTE THIS IS a bit SLOW
...
tmp.df3%>%
subset(Condition == 'IAPS' & ssid ==  353)%>%
  ggplot(aes(x = timerezero3, y= gsr))+
  stat_smooth()+
    # geom_smooth()+
    # geom_()+
...
tmp.df3%>%
subset( tNo == -5)%>%
  ggplot( aes(x = timerezero2, y= gsr, colour = as.factor(ssid)))+
  # geom_point()
      geom_line() +
    # geom_smooth(aes())+
    geom_vline(aes(xintercept = 0), colour = 'red', linetype = 'dashed', size = 2)+
...
tmp.df3 <- smooth_interpolate_pupil(tmp.df3, pupil="pupil", extendpupil="extendpupil",
                                    extendblinks=TRUE, step.first= 'smooth/interpolate',
                                    maxgap=Inf, 
                                    filter = 'moving',
...
tmp.df3<- tmp.df3 %>%
  group_by(ssid, trialUnq,tNo,screencontent) %>%
  smooth_interpolate_pupil(pupil="extendpupil", extendpupil="extendpupil",
                                    extendblinks=TRUE, step.first= 'smooth/interpolate',
                                    maxgap=100, 
                                    filter = 'moving',
